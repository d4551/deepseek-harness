...
| `@deepseek-ai/dsh-tool-shell` | `bash`、`pwsh` | `ctx.tools`、`ctx.shell`、`ctx.systemPrompt`、`ctx.shellEnv`、`ctx.jobs at call time for run_in_background` | `tool/call`、`tool/result` | - | 一个包，每次挂载提供一个面向模型的工具：必填的 `dialect` 配置决定工具名、命令词汇和提示词分节。部署只挂载一次，方言需与为 `ctx.shell` 提供后端的执行器匹配（`bash` 对应 `@deepseek-ai/dsh-bash-local`／`dsh-bash-sandbox`，`pwsh` 对应 `@deepseek-ai/dsh-pwsh-local`／`dsh-pwsh-sandbox`）；本页同时展示两个分支。使用 `run_in_background` 的运行会注册到通用 `ctx.jobs` 运行时，并通过 `job_*` 工具（来自 `@deepseek-ai/dsh-tool-jobs`）收集／停止；禁用 `enableRunInBackground` 配置（默认为 true）后，该参数会被完全移除。pwsh 方言额外给出 Windows 终止说明、原生 `C:\...` 路径和 `$env:NAME` 变量；两个方言都在新进程中运行每次调用，不使用持久 PTY 会话。 |
| `@deepseek-ai/dsh-tool-cordis` | `cordis_define`、`cordis_inspect_list`、`cordis_inspect_query`、`cordis_inspect_self`、`cordis_run`、`cordis_stop`、`cordis_undefine` | `ctx.tools`、`ctx.dynamicCordisRunner` | `tool/call`、`tool/result`、`process-local dynamic package lifecycle` | - | 不在任何随产品发布的树中，需要显式选择启用；动态 Package 代码可以访问真实运行时，见 .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md。该工具集注入 `@deepseek-ai/dsh-cordis-host-runner` 提供的 `ctx.dynamicCordisRunner`，后者拥有定义注册表和 vm 沙箱；组合缺少它时这些工具不会激活。运行中的 Package 在停止、undefine 或 DSH 重启前可以注册**额外的**模型可见工具；发生这类工具集变化时，系统会记录完整且有变动的请求头。 |
| `@deepseek-ai/dsh-tool-shell-persistent` | `bash`、`pwsh` | `ctx.tools`、`ctx.terminals`、`an owning Agent at execution time` | `tool/call`、`PTY shell state`、`tool/result` | - | 每次挂载提供一个按所有者隔离的持久 shell 工具；必填的 `dialect` 配置决定工具名、命令包装和默认描述，本页同时展示两个分支。部署按其 PTY 后端所用的方言挂载一次，提供该后端，并可覆盖面向模型的环境描述。 |
...
来源：[`packages/shell/tool-shell/src/index.ts`](../packages/shell/tool-shell/src/index.ts)
...
一个包，每次挂载提供一个面向模型的工具：必填的 `dialect` 配置决定工具名、命令词汇和提示词分节。部署只挂载一次，方言需与为 `ctx.shell` 提供后端的执行器匹配（`bash` 对应 `@deepseek-ai/dsh-bash-local`／`dsh-bash-sandbox`，`pwsh` 对应 `@deepseek-ai/dsh-pwsh-local`／`dsh-pwsh-sandbox`）；本页同时展示两个分支。使用 `run_in_background` 的运行会注册到通用 `ctx.jobs` 运行时，并通过 `job_*` 工具（来自 `@deepseek-ai/dsh-tool-jobs`）收集／停止；禁用 `enableRunInBackground` 配置（默认为 true）后，该参数会被完全移除。pwsh 方言额外给出 Windows 终止说明、原生 `C:\...` 路径和 `$env:NAME` 变量；两个方言都在新进程中运行每次调用，不使用持久 PTY 会话。
...
来源：[`packages/shell/tool-shell-persistent/src/index.ts`](../packages/shell/tool-shell-persistent/src/index.ts)
...
每次挂载提供一个按所有者隔离的持久 shell 工具；必填的 `dialect` 配置决定工具名、命令包装和默认描述，本页同时展示两个分支。部署按其 PTY 后端所用的方言挂载一次，提供该后端，并可覆盖面向模型的环境描述。
...
使用 team_task_get 或 team_task_list 返回的最新 revision，对共享任务操作执行 compare-and-set。当写入范围与一个已在进行中的任务重叠时，claim、reassign 以及扩大范围的 edit 都会被拒绝。
...
```json
{
  ...
  "properties": {
    ...
    "write_scopes": {
      "type": "array",
      "description": "Replacement write scopes for edit.",
      "items": {
        "type": "string"
      }
    },
