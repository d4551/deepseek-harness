...
- **Coverage gate** (`bun run test:coverage`): requires per-file 100% on `packages/*/*/src`. Uncovered lines may be dead code to delete; executed lines alone do not prove shipped behavior. `vitest.config.ts` records each exclusion and reason: no measurable runtime coverage, execution in Workers/browser realms/subprocesses invisible to unit-process V8, or browser-lane debt marked `DEBT(gui)`, `DEBT(inspector)`, or `DEBT(webworker)`. `bun run verify-coverage-debt` rejects unmarked debt, empty globs, wholly redundant entries and unused markers, including platform-conditional lists in `scripts/vitest-inventory.ts`. It reports per-lane glob and file counts for reduction. `bun run measure-coverage-debt` removes the debt exclusions during measurement, reports each file's shortfall and identifies entries already meeting the bar; trailing arguments select one package's suites.
