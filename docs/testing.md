...
Network-drive tests require the pinned official rclone WebDAV server. Run `bun run test:prepare:network-drive` first. Windows/macOS preparation downloads x64/arm64 binaries to `node_modules/.cache/network-drive-tests`, verifying archive and executable SHA-256 hashes against `packages/fs/fs-network-drive/tests/fixtures/webdav/server.json`; an optional archive-path argument uses identical checks. Linux requires Docker and downloads the image by immutable digest. Both use the same upstream revision. Tests verify native binaries or use `--pull=never`; they never download dependencies. Each test releases its process/container, port and directories after its Cordis context. See [rclone downloads](https://rclone.org/downloads/#beta-releases) and [Docker installation](https://rclone.org/install/#docker-installation).
...
- **Coverage gate** (`bun run test:coverage`): requires 100% statements, branches, functions and lines for every JavaScript/TypeScript file under `packages/*/*/src`. `bun run verify-coverage-debt` checks unconditional per-file thresholds and complete source inclusion against an independent inventory. Debt measurement retains those thresholds and rejects stale or incomplete evidence. Mutation tests establish whether assertions detect behavior changes ([decision](../.agents/notes/implemented/testing/2026-09-16-complete-source-quality-gates.md)).
