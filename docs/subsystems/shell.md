...
The bash execution seam is split across a Service Definition ([dsh-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([dsh-bash-local](../../packages/shell/bash-local) and [dsh-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([dsh-tool-shell](../../packages/shell/tool-shell), the `bash` or `pwsh` schema its `dialect` config selects). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `dsh-tool-shell`. The local executor scrubs ambient credentials before merging explicit caller-supplied env. See [the bash-stdin-env Agent Note](../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md).
...
`start()` returns a handle with no id or owner. `dsh-tool-shell` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the process closes and never rejects, reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
...
`ShellExecutor` owns `resolve`, foreground `run`, background-process `start`, and the `sandboxMode` capability fact. `dsh-shell`'s `SubprocessShellExecutor` owns command defaulting, timeout/abort classification, output budgets, the background read merge, and the `ctx.sandbox` confinement layer; `dsh-bash-local` and `dsh-pwsh-local` add only their dialect — the argv, the terminal environment, and the diagnostic name — and their confining subclasses only turn confinement on. Process groups, bounded collectors, spill files, credential scrubbing, and disposal quiescence are the [subprocess service](subprocess.md)'s. The tool Consumers adapt background handles into the [generic job runtime](jobs.md) and state each tool's own name, description, and command vocabulary. `dsh-shell` owns everything both tools say identically: `renderShellResult`/`renderShellProcessRead` produce the model-facing result text, the parameters and output schema are one declaration, and `parseExitStatus`/`ParsedExitStatus` inverts the `[exit code: N]` / `[killed by signal: X]` markers so both tools' `presentResult` split the rendered text into the terminal card's output body and its exit-status pill.
