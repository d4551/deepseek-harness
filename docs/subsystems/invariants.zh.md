...
[dsh-invariants](../../packages/runtime-diagnostics/invariants) 是面向包自有运行时不变式检查的可配置注册表服务（`ctx.invariants`）。注册表拥有选择逻辑、名称保留、子 fiber 生命周期和归因到包的失败。每个工作区包发布一个 `./invariant` 配套插件，以自己确切的 npm 包名注册检查。检查断言权威事件流或可变数据，绝不是服务或方法是否存在。[不变式服务 Agent Note](../../.agents/notes/implemented/architecture/2026-07-19-package-owned-invariant-service.zh.md)规定注册表设计。
...
每个工作区包都拥有一个 `./invariant` 配套插件。配套插件检查所属包拥有的可观察事件或可变数据关系。`bun run verify-package-invariants` 拒绝「生成文件」标记、所有空安装器、遗漏或忽略报告器的非空安装器、错误的注册名称，以及不完整的导出、发布、依赖或打包接线。说明性注释不构成豁免，通用存在性断言也不能构成有意义的运行时检查（[门禁决策](../../.agents/notes/implemented/process/2026-09-16-reject-empty-invariant-installers.zh.md)）。可执行配套插件的目录与标准组合方式见[包 README](../../packages/runtime-diagnostics/invariants/README.zh.md)。
