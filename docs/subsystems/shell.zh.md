...
bash 执行 seam 分为 Service Definition（[dsh-shell](../../packages/shell/shell)，`ctx.shell`）、Service Provider（[dsh-bash-local](../../packages/shell/bash-local) 与 [dsh-bash-sandbox](../../packages/shell/bash-sandbox)）和 Consumer（[dsh-tool-shell](../../packages/shell/tool-shell)，即由其 `dialect` 配置选定的 `bash` 或 `pwsh` schema）。通用后台任务的 job id、所有权与控制位于 [jobs.md](jobs.zh.md)；本 seam 返回一个不含任务概念的进程句柄。原始进程组机制封装在[子进程 seam](subprocess.zh.md)之后。
...
`stdin` 和 `env` 是受信任的进程内插件输入，不由 `dsh-tool-shell` 暴露。本地执行器会先清除环境中的凭据，再合并调用方显式提供的 env。见 [bash-stdin-env Agent Note](../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.zh.md)。
...
`start()` 返回不含 id 或所有者的句柄。`dsh-tool-shell` 将它适配为 `ctx.jobs.start()` 钩子；随后由通用运行时拥有任务标识与生命周期。`done` 在进程关闭时完成且绝不被拒绝；进程结束后仍可读取，并且沙箱事实会在 `done` 完成前写入。
...
`ShellExecutor` 拥有 `resolve`、前台 `run`、后台进程 `start` 以及 `sandboxMode` 能力事实。`dsh-shell` 的 `SubprocessShellExecutor` 拥有命令默认值补全、超时/中止分类、输出预算、后台读取合并以及 `ctx.sandbox` 约束层；`dsh-bash-local` 与 `dsh-pwsh-local` 只补充各自方言——argv、终端环境与诊断名称——其沙箱子类也只是启用约束。进程组、有界收集器、spill 文件、凭据清除与 dispose（资源释放）后完全停稳归[子进程服务](subprocess.zh.md)所有。工具 Consumer 把后台句柄适配到[通用任务运行时](jobs.zh.md)，并为每个工具声明其名称、描述与命令词汇。两个工具表述完全一致的部分都归 `dsh-shell`：`renderShellResult`/`renderShellProcessRead` 产出面向模型的结果文本，参数与输出 schema 只有一份声明，而 `parseExitStatus`/`ParsedExitStatus` 是 `[exit code: N]` / `[killed by signal: X]` 标记的逆解析，两个工具的 `presentResult` 都用它把渲染文本拆分为 terminal 卡的输出正文与退出状态 pill。
