...
export function runPlugin(profile: string, args: readonly string[]): number {
  ...
  } else if (ensureProfileInstallSettings(dir)) {
    // A profile created before this installation used bun has no bunfig.toml,
    // and initialization is skipped for a directory that already has a
    // manifest, so the settings are written here instead.
    process.stderr.write(`${NAME}: wrote missing bun install settings for profile ${profile} at ${dir}\n`)
  }
  const before = readProfileManifest(NAME, dir)
  // bun ships a real executable on every supported platform, so this needs no
  // shell — which keeps argument quoting and signal behaviour identical across
  // platforms rather than reintroducing the .cmd-shim shell hop.
  const result = spawnSync('bun', args.map(argument => anchorPathSpec(argument, process.cwd())), {
    cwd: dir,
    stdio: 'inherit',
  })
