...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  if (SKIP_IN_RECORD_MODE) console.info('[skip] replay-round-trip.e2e.ts: record mode refreshes fixtures; this replay-only assertion runs in replay/refresh')
  it.skipIf(SKIP_IN_RECORD_MODE)('rendered the settled turn: markdown, tool row, composer restore', async () => {
  ...
  if (SKIP_IN_RECORD_MODE_2) console.info('[skip] replay-round-trip.e2e.ts: record mode refreshes fixtures; this replay-only assertion runs in replay/refresh')
  it.skipIf(SKIP_IN_RECORD_MODE_2)('matches the conversation aria golden with stable anchors', async () => {
  ...
  if (SKIP_IN_RECORD_MODE_3) console.info('[skip] replay-round-trip.e2e.ts: record mode refreshes fixtures; this replay-only assertion runs in replay/refresh')
  it.skipIf(SKIP_IN_RECORD_MODE_3)('renders the system prompt disclosure inside the expanded Turn process', async () => {
  ...
  if (SKIP_IN_RECORD_MODE_4) console.info('[skip] replay-round-trip.e2e.ts: record mode refreshes fixtures; this replay-only assertion runs in replay/refresh')
  it.skipIf(SKIP_IN_RECORD_MODE_4)('expands and collapses the reasoning fold from its click target', async () => {
  ...
  if (SKIP_IN_RECORD_MODE_5) console.info('[skip] replay-round-trip.e2e.ts: record mode refreshes fixtures; this replay-only assertion runs in replay/refresh')
  it.skipIf(SKIP_IN_RECORD_MODE_5)('stayed clean: no pageerrors, no reconnect self-healing, no server errors', async () => {
