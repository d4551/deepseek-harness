...
// graph itself. Other files share the same scaffolding (assembled-boot.client.ts) to
...
import { installAssembledBootEnv, mountAssembledApp } from './assembled-boot.client.ts'
...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    const buildVersion = commit === undefined ? version : `${version}-${commit}`
