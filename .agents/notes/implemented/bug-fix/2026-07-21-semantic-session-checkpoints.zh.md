...
`dsh-session-checkpoint-policy` 以零配置插件的形式与持久化后端共同加载，并负责语义持久性屏障。在 `agent/pre-step` 时，该插件会在推导下一个请求前刷新待持久化的提示词输入或前一批响应/结果。它参与强制并行的 `llm/request-ready` 事件，在记录 `request/header` 之后、构造流式中间件之前刷新活动会话。直接与已准备的调用都会在首次迭代时进入就绪过程。该插件还在有序的执行前策略之后包装顶层 `tools/execute`，在进入工具主体前刷新已记录的 `tool/call`；嵌套分发则复用外层模型可见调用。循环的最终 `turn/end` 检查点仍是轮次的收尾边界，并会在处理另一个已排队轮次或观察到空闲状态之前完成。
...
就绪过程持有所有已启动的监听器直到结算，取消后也不例外，并将其失败一起作为 `AggregateError` 报告。就绪成功后，已取消的请求会产出 `ABORTED` 结束分片，不进入流式中间件。首次迭代前已取消的请求不会启动就绪工作。已准备的调用在构造 `stream()` 时同步占用唯一一次使用机会；回放只会在就绪后分派到达它时绑定并推进。就绪失败不会消耗回放条目，逆序迭代遵循分派顺序而非构造顺序。
...
**在流式中间件内执行检查点**无法强制就绪：先于检查点注册的回放监听器可以直接返回自己的流，而不调用 continuation。强制并行事件会让所有就绪监听器参与，并在构造该 waterfall（瀑布式事件）之前等待它们全部完成；监听器注册顺序无法让回放绕过持久化。

## 后果

[持久回放回归测试](../../../../packages/session/session-checkpoint-policy/tests/replay-durability.spec.ts)用目录阻塞真实 JSONL 路径，并验证原生写入失败。无论监听器以哪种顺序注册，直接与已准备的调用都会在消耗回放前拒绝；恢复存储介质后，原始条目可以分派，请求头也已持久化。额外的真实 Cordis 监听器屏障验证取消和失败时的完整结算。这些检查覆盖请求持久性，不涉及附件隔离或已准备图片字节的所有权。
