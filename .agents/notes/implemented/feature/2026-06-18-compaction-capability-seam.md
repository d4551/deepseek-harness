...
### Head-anchoring with checkpoint-aware selection

Auto-compaction starts at the surface head, but range selection is checkpoint-aware: when exactly one existing checkpoint leads the surface, the pass starts after it and summarizes only the newer content; two or more leading checkpoints, or newer content too small to summarize usefully, consolidate from the head back into one. A pressure pass keeps condensing toward a hysteresis watermark below the threshold instead of stopping just under the trigger. `shadowedRange` is therefore positional rather than a numeric sequence interval: a newer summary sequence may occupy an older surface position. `shadowedSeqs` records the authoritative surface order. Manual mid-range compaction may leave multiple checkpoints. The policy and its rationale live in the [checkpoint-aware auto-compaction Agent Note](2026-09-01-checkpoint-aware-auto-compaction.md).

### Approximate convergence invariant

`resolveConfig` supplies usable defaults: threshold ratio `0.8`, target watermark ratio `0.85`, retained-tail ratio `0.16`, empty summarization provider/model overrides, `maxTokens: 8192`, `compactionRetries: 1`, `maxOverflowRetries: 1`, and `auto: true`. Optional exact provider/model policies partially override the top-level defaults; pressure scales ratios against capacity from the route-owning LLM provider service, while `retainTokens` can replace ratio retention. Retention must remain below the resulting threshold and below the target watermark. Convergence remains dynamic because provider output caps can be spent on hidden or surfaced reasoning tokens and summary size is unpredictable. A pressure pass condenses toward the watermark: the first pass skips one leading checkpoint when newer content alone is worth a call, later passes consolidate from the head, and a summary that does not shrink its span retries as consolidation within the retry budget. Each committed summary must be smaller than what it shadows. Overflow needs no capacity metadata and bypasses threshold and retained-tail policy for one maximal balanced head reduction, leaving the newest indivisible unit. The ownership split is specified by the [routed model context and compaction policy Agent Note](../architecture/2026-07-20-routed-model-context-and-compaction-policy.md).
