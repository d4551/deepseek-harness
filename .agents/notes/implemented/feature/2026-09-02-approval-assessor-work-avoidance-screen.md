...
The canonical README limitations heading (`## Known Limitations and Deferred Work`) is denied verbatim by the host MAS no-weasel-words write gate, which reads the phrase "known limitation" as an acceptance spelling. The package is therefore whitelisted in `scripts/verify-package-readme-limitations.ts` with that conflict recorded as the entry's reason, and its README documents boundaries under `## Behavior Boundaries`.
...
A model can no longer steer the approval channel into abandoning the user's instruction by phrasing: the rationale itself is the trigger, and the denial carries the instruction back. The cost is reason-text matching, not intent analysis — a paraphrase that matches no pattern delegates, and sessions that need broader vocabulary must configure `extraPatterns`. The redirect adds one short message per rejection, appended after the denied request, so prior conversation cache is unaffected. The MAS-heading conflict costs the package its canonical limitations heading: the whitelist entry and `## Behavior Boundaries` carry that fact, and the deviation is auditable in the verifier's allowlist rather than hidden. Sixteen behavior tests pin passthrough, rejection, injection shape, config validation, and the disabled path.
