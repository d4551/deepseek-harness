...
准入取消会移除排队工作，正在运行的原生操作则保持占用压缩槽位直到结束。每个批次都会等待全部准备结果后才拒绝，因此返回拒绝后不会再有准备工作在后台运行。发布在原子文件系统操作开始前接受取消；开始后则完成持久化流程。取消批次中较后的成员不会删除较早提交的不可变对象，因为其他会话可能已经共享这些对象。
...
共享请求取消使用其他调用方监听器无法阻断的中止监听器。每个等待方保留自己的取消原因。最后一个等待方取消时，会在返回前等待共享操作结束，因此所有调用方都结束后不会再有缓存写入或原生变换继续运行。压缩队列接收共享信号，原生编码器在候选之间停止。缓存未命中或图片格式损坏时可以重新生成；文件系统读取失败会直接传播，不会触发无关的编码与替换尝试。
...
**调用方取消时立即释放原生压缩容量。** 尚未结束的 Sharp 操作仍会消耗原生资源。提前释放槽位会让反复取消突破配置的并发上限；保持所有权直到操作结束可以约束实际工作量，代价是取消延迟。
...
原生文件系统与 Sharp 测试覆盖排队和运行中的准入取消、槽位所有权，以及图片未被发布。真实 Loader 组合运行构建后的附件提供方，取消批次并验证存储仍为空，随后成功保存并读取图片。原子操作开始后的发布取消仍缺少原生平台故障注入覆盖。
