...
- 无密钥 SDK 快照场景 `subagent-list-agents` 曾使用仅限快照的 `subagent/end` 标记为第二个 parent 轮次设置边界，并针对 subagent 服务、投影注册表和 JSONL 持久化真实执行 `list_agents`，渲染 `<id> [ready] — <label>`，直到 2026-09-07 Team 工具在每个 profile 中接管 `list_agents`、该场景随之退役（[note](../architecture/2026-09-07-agent-teams-in-every-profile.zh.md)）；`packages/subagent/tool-subagent-control/tests/list-agents.spec.ts` 继续覆盖该渲染。
