...
- 没有任何无密钥 SDK 场景运行 `list_agents`：`subagent-list-agents` 已退役，因为 Team 工具在每个随发布 profile 中拥有 `list_agents`（[Agent Teams 随每个 profile 交付](../architecture/2026-09-07-agent-teams-in-every-profile.zh.md)）；`packages/subagent/tool-subagent-control/tests/list-agents.spec.ts` 针对真实的 subagent 服务渲染 `<id> [ready] — <label>`。
