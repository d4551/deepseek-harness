...
Selected rows take `css.multiSelected` — the hover fill plus a leading accent rule, so a run reads as one block next to the single current-Session highlight it can overlap — and report `aria-selected`; both list containers declare `aria-multiselectable`. The open Session reports `aria-current` instead, because a tree that declares multi-selection has to keep the navigation highlight apart from a range member, or the set a reader hears as selected is not the set a bulk archive commits. A row no range can reach — the Ungrouped bucket, a blank draft — reports neither. A visually hidden `role="status"` reports the reached Session count, since the marking is the only other channel.
...
**Let a selected header contribute its members without marking them.** Rejected: a header caught in the middle of a range would archive sessions below the operator's shift-click, outside anything highlighted. Driving the marking from the resolved reach keeps every rendered member of the committed set visible; the count carries the rest, because a folded group renders no session rows to mark.
