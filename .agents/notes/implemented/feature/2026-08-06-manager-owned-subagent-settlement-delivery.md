...
No recorded scenario pins a failure: `subagent-continuable` is retired because the Team tools own `send_message` in every shipped profile ([Agent Teams ship in every profile](../architecture/2026-09-07-agent-teams-in-every-profile.md)). `packages/subagent/subagent/tests/run-settlement.spec.ts` maps each stop reason onto its Task outcome, and the `subagent-published-run-failure` session scenario keeps a forced durability failure in a recorded transcript.
