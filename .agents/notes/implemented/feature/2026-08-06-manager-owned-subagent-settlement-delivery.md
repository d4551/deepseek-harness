...
`subagent-continuable` was the one that pinned a failure until the scenario was retired on 2026-09-07, when the Team tools took over `send_message` in every profile ([note](../architecture/2026-09-07-agent-teams-in-every-profile.md)). Its child's last turn died on the forced durability checkpoint without entering a step, so that transcript was where the stop-reason rule above was visible end to end: the notice said the child *failed*, carried the earlier `SECOND_OK` as its last content rather than as a result, and the parent's own acknowledgement turn reached the ACP client. The settlement suites under `packages/subagent/subagent/tests/` keep the rule covered.
