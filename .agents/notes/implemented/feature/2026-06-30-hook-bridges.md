...
- **Stop loop-guard.** Shipped 2026-09-08 in the shared `registerTurnStoppingHook`: both dialects report `stop_hook_active: true` from the second Stop run in one turn, and the eighth consecutive block in a turn is overridden with a warning — Claude Code's cap, applied to Codex too since it documents none.
- **Hook `continue:false` (hard halt).** Shipped 2026-09-08: the shared extension points end the turn through the agent's `hook` cancel cause where each dialect applies it (Claude Code on `PreToolUse`, where the denial carries the stop reason, and on `UserPromptSubmit`; Codex on `UserPromptSubmit`, while its `PostToolUse` stop replaces the tool result and continues); a `Stop` halt outranks a block in both, and `stopReason` reaches the model's next request on Claude Code only.
...
- **Session-start context is gated (shipped 2026-09-08); subagent-start context stays best-effort.** The first pre-step after a session start waits for the detached `SessionStart` run and carries its context into that request, so a slow hook delays the first request instead of missing it; the subagent-start hook still injects when ready and may miss a short-lived child.
