...
没有任何录制场景固定失败结局：`subagent-continuable` 已退役，因为 Team 工具在每个随发布 profile 中拥有 `send_message`（[Agent Teams 随每个 profile 交付](../architecture/2026-09-07-agent-teams-in-every-profile.zh.md)）。`packages/subagent/subagent/tests/run-settlement.spec.ts` 把每个终止原因映射到其 Task 结局，`subagent-published-run-failure` session 场景则在录制 transcript 中保留一次被强制的持久性失败。
