...
The lane covers `ui-primitives` only. The composed surfaces — chat, settings, workspace — are where landmark, heading-order, and labelling defects usually appear, and they are not audited here; each needs its feature suite's context to render, which is the work this note leaves open.
