...
- **`break` sits one mutant below the recorded score, and only moves up.** The scope measures 96.45 — 780 killed plus 7 timed out of 816 reachable mutants. One mutant is worth 0.1225 points here, so the threshold is 96.4: losing a single kill scores 96.32 and fails the run.

## What the survivors are

The 29 survivors are not missing assertions; they are mutants no assertion over the public result can reach, and the distinction is what keeps the number meaningful:
