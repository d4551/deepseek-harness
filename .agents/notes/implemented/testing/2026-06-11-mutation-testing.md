...
- **`break` sits one mutant below the recorded score, and only moves up.** The scope measures 99.08 — 750 killed plus 7 timed out of 764 reachable mutants, recorded by the [survivor-repair round](2026-08-30-mutation-survivor-repair.md). One mutant is worth 0.131 points here, so the threshold is 98.9: losing a single kill scores 98.95 and stays above it; losing two scores 98.82 and fails the run.
...
- **Guards subsumed by the arithmetic beside them were deleted, not annotated.** The `i < 0` and `expected === 0` returns in the trailing-sequence trimmer fell to the [survivor-repair round](2026-08-30-mutation-survivor-repair.md): once the final length comparison carries the same contract, the early returns are unreachable dead code whose mutants are permanently unkillable. That round records why the deletion is not the removal-chasing this decision refused — the spec now lives in the comparison the guards duplicated.
...
99 needs the survivors at or under 1% of the reachable mutants — seven of 764. There are exactly 7, and they are the equivalent floor listed above: every remaining survivor is an alternate form of correct code (a falsy-versus-undefined catch, a loop boundary whose mutant appends an empty value, an unexposed fold direction, an unexposed regex block, an out-of-bounds walk exit, and a lead byte the walk cannot stop on). Killing one means either deleting code that is correct and intentional — the thing this note exists to refuse — or widening the scope.
...
Both measured scopes sat above the line when this decision was written; the survivor-repair round drove this one to 0.92% (7 of 764), under it, which is what let `break` reach 98.9. `tool-todo`, measured as a second scope, arrived at 12.00% and reached 4.00% after tests that pinned its whole model-facing description, its parameter and output schemas, `counts.completed`, the companion's plugin name, and the turn boundary across an unrelated event — 22 mutants, every one a contract nothing had asserted. Of its remainder, four are provably equivalent: a `typeof` guard the status-set check already subsumes, `{}` for `{ open: false }` where the field is only read for truthiness, and the trace cache, which recomputes the same trace when it is skipped.
...
Surviving mutants are work items with a shape: pick one, write the test that kills it, repeat. That loop took this scope from 82.92 to 96.57, the exact-window suffix refactor then took it to 96.92 while driving `timeout` to 100, and the survivor-repair round took it to 99.08 with `break` at 98.9 — producing tests for real behavior that had none — UTF-8 cut boundaries, LaunchServices parsing, temp-file exclusivity, name folding, the writer lock's deadline and backoff, a thrown null reaching its caller intact, mkdir option pinning, and the home-paths root guard — plus code simplifications the survivors exposed, and one narrowed `try` that had been swallowing this package's own parser failures as an absent macOS record.
