...
**Teach the repository wrapper to load a bundle patch.** Rejected because the repository cache and preparation protocol would still duplicate profile dependency installation. Bundle packages are already accepted from npm, Git, file, and link specifications through bun.
...
- Profile installation requires `bun` on the host `PATH`. This is acceptable for an explicit package-management operation and avoids shipping the removed cache's pinned package-manager runtime solely for configuration-time activation.
...
- The dedicated package-manager runtime, preparation executable, wrapper generator, Git credential CI setup, repository cache, and repository-specific tests disappear.
