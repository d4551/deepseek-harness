...
A module that declares its own API and also names a few forwarded symbols is not a barrel. It is the module a package's `exports` map points at, and the gate leaves it alone. The line the detector draws is ownership, not re-export count: a module with no declarations of its own has no claim to the names it publishes.
