...
- **Package resolution retains the host base.** The mount configuration carries the host's required `baseUrl` independently of Include's preset-directory context. A missing native resolver rejects bare packages rather than resolving from a different installation; file paths, file URLs and builtins retain their own resolution. Native child-process tests exercise both addon-enabled and addon-disabled Node hosts with competing package installations.
- **A failed mount owns teardown through settlement.** The failure path retains the startup or audit error, joins an unload already started by a rejecting disposer, and reports cleanup failures alongside the original cause. A real status-observer failure and a gated asynchronous disposer verify that rejection cannot precede owned cleanup or discard either cause.
- **A subtree owns its mount record.** Successful validation registers the record through the subtree's effect lifecycle. Unload releases it without requiring a later registry read or mount, so an idle process cannot retain the last disposed tree through the registry. Reads exclude disposed fibers before asynchronous cleanup settles. A native garbage-collection regression verifies release after real plugin resource cleanup.
...
- **A settled mount serves until its composition file's stamp changes.** The composition a running session joined must survive its file changing or disappearing; each generation records the file's stamp (mtime + size) and a session that finds it stale starts the next generation, so file edits — the only composition editor once authoring became copy-only — reach later sessions without any authoring call dropping the pointer. Joined sessions keep their generation; since 2026-09-08 a superseded generation is released once its last joined session is disposed, and at once when none joined, so an edit no longer pins a subtree for the life of the process.
