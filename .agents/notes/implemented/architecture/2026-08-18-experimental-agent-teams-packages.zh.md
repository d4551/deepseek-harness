...
`packages/subagent/agent-team`、`packages/subagent/tool-agent-team`、`packages/preset/agent-team-profile`、`packages/client/ui-agent-team` 与 `packages/preset/agent-team-web-profile` 是 Agent Teams 包；它们曾作为 `packages/experimental/` 下的私有 workspace 孵化，现按本记录定义的 promotion 流程进入各自产品职责组。[实验性包命名决策](2026-08-19-experimental-package-name-prefix.zh.md)负责其 npm 名和 promotion 重命名；本记录负责其目录归属、发布排除与依赖隔离。
