...
`dsh-base` mounts `agent-team` and `tool-agent-team` for every profile, with the delegated policy, the eight-member roster, and the same mailbox and disposal limits the swarm layer restated. The policy tells the model to create teammates only when the user asks, so an ordinary session reads the Team tools without being steered into a swarm. `dsh-web-app` mounts `ui-agent-team` after `ui-subagent`, so every Web profile renders the roster, task board, and teammate navigation, and the Agent team card on the Plugins settings page renders wherever the `agent-team` namespace is served, which is now everywhere. One agent preset stays outside: `minimal` promises exactly persistent bash and the string editor, so base sets `tool-agent-team.excludePresets` to `minimal`. Admission follows the actual joined composition and its changes, not the immutable creation header. An excluded or not-yet-composed Agent receives neither Team tools nor the policy section. Without a preset service, the session header controls this restriction. The Web bundle's other presets choose their delegation tools themselves and keep the Team.

The [preset admission tests](../../../../packages/subagent/tool-agent-team/tests/preset-admission.spec.ts) exercise real composition changes, repeated registration and disposal, and a prepared task mutation whose authority disappears before dispatch. The [shipped Web composition tests](../../../../apps/cli/tests/web-agent-presets.e2e.ts) call the executor across both switch directions. Tool removal must revoke execution as well as presentation; changing the displayed schemas alone would leave an already prepared call authorized.
