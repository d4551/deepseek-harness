...
- **包解析保留宿主基址。** 挂载配置携带宿主必需的 `baseUrl`，独立于 Include 使用 preset 目录的上下文。原生解析器缺失时会拒绝裸包名，而不会从另一处安装解析；文件路径、文件 URL 与内建插件保留各自的解析方式。原生子进程测试使用存在同名包安装的宿主与 preset 目录，分别验证允许和禁用 addon 的 Node 宿主。
- **失败的挂载拥有卸载操作直到结算。** 失败路径保留启动或审计错误，等待拒绝的释放操作已经启动的卸载，并将清理失败与原始原因一起报告。真实的状态观察器错误和受控等待的异步释放操作验证：拒绝不能早于所拥有的清理完成，也不能丢弃任一原因。
- **子树拥有自己的挂载记录。** 校验成功后，记录通过子树的 effect 生命周期注册。卸载会释放记录，无需后续注册表读取或挂载，因此空闲进程不会通过注册表保留最后一棵已释放的树。异步清理结算前，读取也会排除已释放的 fiber。原生垃圾回收回归测试在真实插件资源清理后验证记录已释放。
...
- **挂载一旦成功即持续供职，直到组装文件的 stamp 变化。** 运行中会话加入的组装必须在其文件被修改或删除后继续存活；每个代际记录文件 stamp（mtime + 大小），会话发现当前代际已陈旧时，会开启下一个代际，因此文件编辑——创作改为仅复制之后唯一的组装编辑器——无需任何创作调用丢弃指针即可达到后续会话。已加入的会话保持其代际；自 2026-09-08 起，被替代的代际会在其最后一个已加入会话被释放时释放，若从未有会话加入则立即释放，因此一次编辑不再把子树钉住直到进程结束。
