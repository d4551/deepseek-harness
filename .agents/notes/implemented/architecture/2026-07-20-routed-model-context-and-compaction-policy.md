...
Compact-basic owns consumer policy. Top-level fields define defaults; `modelPolicies` contains partial overrides keyed by the exact `{ provider, model }` pair. Duplicate targets and unknown or invalid fields fail plugin load. `thresholdRatio` defaults to `0.8`, the hysteresis watermark `targetRatio` defaults to `0.85`, and retention defaults to `retainRatio: 0.16`; callers may use an absolute `retainTokens` instead, but the two retention forms are mutually exclusive. After inheritance, a ratio retention that is not below its threshold ratio, or not below `thresholdRatio × targetRatio`, fails plugin load because no model capacity can make that policy valid.

For proactive pressure, compaction-basic reads the latest durable request route, resolves its provider-service capacity and exact-target policy, and scales ratios into a `ResolvedCompactSpec` carrying the threshold and the hysteresis target watermark. It performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately. An absolute retained budget that is not below the scaled threshold, or not below the watermark, fails when the target capacity first makes that comparison possible.
