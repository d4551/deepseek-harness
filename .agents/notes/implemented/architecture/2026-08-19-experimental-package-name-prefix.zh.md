...
Agent Teams 曾以 `@deepseek-ai/dsh-experimental-agent-team` 与 `@deepseek-ai/dsh-experimental-tool-agent-team` 孵化，promotion 重命名后现发布为位于 `packages/subagent/agent-team` 的 `@deepseek-ai/dsh-agent-team` 与位于 `packages/subagent/tool-agent-team` 的 `@deepseek-ai/dsh-tool-agent-team`。包 import、Cordis 配置项、生成目录和仓库元数据直接使用这些名称，不提供兼容别名。
