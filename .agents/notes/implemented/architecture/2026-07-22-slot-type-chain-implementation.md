...
In the type chain, a chain entry's SlotMap shape is `{ kind: 'chain'; scope; owner }` with `owner` as the chain's currency; `M` — the `matched` prop's type — is inferred from the select return (a selector narrowing a union member types `matched` automatically), and `NoInfer` keeps the component position out of `M` inference (rulings below). On the owner side, `renderSlotChain(key, owner, { fallback })` joins `renderSlot` in the `PropsRenderSlots` share, its key domain statically narrowed to the chain-kind keys of the entry's children declaration (`ChainKeysOf`); the dispatch site is one line and holds no derivation or routing logic of its own.
...
Three hardening decisions in the register signature exist because the obvious alternative fails in a specific, reproducible way; a future editor should not re-litigate them:
...
2. **`InjectSeat` derives the `inject` requirement from the component, inside one `register` signature.** The business share cannot be pinned the way `M` is: an inject factory written `(sessionId) => …` has an unannotated parameter, so it is context-sensitive and its return type reaches inference one round late — a `NoInfer<I>` at the component position fixes `I` at its `object` default first, and every call then rejects the face it was given. `I` is therefore inferred from the component, and the options seat reads the demand back off it: while `FrameworkProps<…>` (the composed props with an empty face) still satisfies the component's parameter, `inject` stays optional; the first member the framework shares do not cover makes the factory mandatory with its return pinned to `I`. A drifted component still widens `I` — and that widening is what makes the seat demand a factory the call does not have, so the drift surfaces as a missing `inject` on the options argument instead of being absorbed.
3. **One signature, not an overload pair.** The seat replaced two `register` overloads whose only difference was the inject share; sharing a type-parameter list between overloads is not expressible, so the pair stood as a self-clone that `bun run duplication` reported. `packages/client/ui-slots/tests/type-chain.host.spec.ts` compiles the complete valid chain against the source contracts and requires the expected compiler diagnostic at each invalid call, including business props without `inject`. `packages/client/ui-conversation/tests/views-type-chain.client.spec.tsx` checks the conversation components' type chain.
