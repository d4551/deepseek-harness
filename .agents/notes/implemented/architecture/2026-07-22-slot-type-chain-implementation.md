...
3. **One signature, not an overload pair.** The seat replaced two `register` overloads whose only difference was the inject share; sharing a type-parameter list between overloads is not expressible, so the pair stood as a self-clone that `bun run duplication` reported. The negative-sample specs pin that the fold lost no strictness: `packages/client/ui-slots/tests/type-chain.client.spec.tsx` and `packages/client/ui-conversation/tests/views-type-chain.client.spec.tsx` turn red with unused `@ts-expect-error` directives the moment the seat is flattened to a plain optional `inject`.
