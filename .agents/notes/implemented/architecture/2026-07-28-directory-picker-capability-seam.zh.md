...
- **在 `DirectoryListing` 上增设线上 `separator` 字段（宿主标注 `path.sep`）。** 已于 2026-09-08 落地：列表携带后端的 `path.sep`，browse 客户端逐字读取它，早先的 `listing.home` 反斜杠启发式——含反斜杠的 POSIX 家目录会击穿它——已经移除。
