...
`packages/subagent/agent-team`, `packages/subagent/tool-agent-team`, `packages/preset/agent-team-profile`, `packages/client/ui-agent-team`, and `packages/preset/agent-team-web-profile` are the Agent Teams packages; they incubated as private workspaces below `packages/experimental/` and now sit in their product-role groups under the promotion procedure this note defines. The [experimental package naming decision](2026-08-19-experimental-package-name-prefix.md) owns their npm names and promotion rename; this note owns their placement, release exclusion, and dependency isolation.
