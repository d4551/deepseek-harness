...
**A `!!js` `name` is reported as the tag it is.** `disabled` and a plugin's `config` are the only places the Loader interpolates, so an expression written for `name` reaches `tree.import` as the node the parse produced and can never resolve. The shape check refused it already, under the reason for an absent `name`: that reason states what the row lacks rather than what it wrote, and an author reading it goes looking at the one row that does carry a name.
