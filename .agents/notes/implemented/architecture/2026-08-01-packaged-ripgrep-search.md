...
- The `@vscode/ripgrep` dependency adds its MIT row to the runtime tier, and the isolated linker's truncated virtual-store directory names needed a content-scan fallback in the notices generator's metadata lookup.
