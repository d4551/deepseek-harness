...
配套两处：根 `package.json` 增补 devDependency `istanbul-lib-report`（isolated linker 的严格布局下 `scripts/` 摸不到嵌套依赖）；`knip.json` 根 workspace 的 entry/project 通配增加 `scripts/**/*.cjs`，使该文件及其依赖对 hygiene 门禁可见。
