...
- Executable `pnpm …` verbs across CI, GitLab CI, lefthook, package scripts, release scripts, and documentation become their bun spelling. `.gitignore` swaps `.pnpm-store/` for the bun equivalents. Three categories keep the word deliberately: vendored READMEs, whose upstream examples the Vendoring Policy leaves untouched; archived Agent Notes, which the archive policy freezes; and client test fixtures whose sample strings depict a command a product user ran in their own project rather than this repository's tooling.
