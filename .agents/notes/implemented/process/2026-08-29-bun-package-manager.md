...
Adopt **bun 1.3.11**, pinned via the `packageManager` field; CI installs that pin through `oven-sh/setup-bun` with `bun-version-file: package.json`. The product runtime stays Node — the single-exe build, `node-pty`, and the SEA carrier are Node artifacts, and `engines.node` is unchanged. The launchers are not untouched, though: `dsh plugin` forwards to `bun` instead of `pnpm`, and a profile's install settings move from a `pnpm-workspace.yaml` the launcher wrote to a `bunfig.toml` it writes now. Profile initialization is skipped for a directory that already has a manifest, so `ensureProfileInstallSettings` writes those settings for a profile created before this change; without it such a profile would install out-of-tree plugins under bun's default isolated layout and lose the flat `node_modules` they resolve peers through. The now-inert `pnpm-workspace.yaml` is left in place rather than deleted from a user's directory.
...
- **What still reads `pnpm`, counted rather than characterised.** 204 archived Agent Notes, frozen by the archive policy. 147 implemented Agent Notes, whose recorded commands are stale: that is outstanding work, not a decision. Four client test fixtures whose sample strings depict a command a product user ran in their own project. Two comments stating what corepack shims and what Stryker's `packageManager` accepts, which are facts about those tools rather than about this repository. No vendored file mentions it at all — an earlier version of this list claimed a Vendoring Policy exemption that covers nothing. Three records name it because it is their subject: this note, [the audit-loop record](../../../audit-loop.md), and the gate below.
