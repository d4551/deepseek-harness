...
The packed-image loadability suite left this roster: it now runs as the dedicated `built-image-specs` gate over emitted `lib/` through [the built vitest lane](2026-08-27-built-image-vitest-lane.md), entirely outside the coverage aggregate, and its build-independent half rejoined the instrumented unit lane.
...
- Native Windows keeps the packed-image suites on a complete workspace artifact tree: the [`built-image-specs` gate](2026-08-27-built-image-vitest-lane.md) declares `build` as its need.
