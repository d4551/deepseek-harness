...
The Go compiler runs no V8 heap, so `build:lib:host` and the Wine build gate invoke `tsc` with no `--max-old-space-size`, and `scripts/ci-workflow.spec.ts` rejects that flag in either file. The compatibility-package ban greps the whole tracked tree with no path excluded, so the root manifest, `bun.lock`, and the `goal/` plan records are covered.
