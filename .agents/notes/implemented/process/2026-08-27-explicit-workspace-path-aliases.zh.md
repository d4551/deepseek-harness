...
另外几个是被覆盖断言揪出来的**整包**：`dsh-client-ui-directory-picker-browse` 与 `dsh-client-ui-directory-picker-native`。它们都叫 `dsh-<分组>-<目录>`，任何通配符都代不出这种形态；而它们身旁的同族包都有手写别名——它们只是漏了。现在补上。
