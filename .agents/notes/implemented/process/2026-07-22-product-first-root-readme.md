...
The root README is a compact product and contributor entry point. It states the product identity and plugin architecture, links the documentation site, names this checkout as a fork of [the upstream repository](https://github.com/deepseek-ai/deepseek-harness) with its bun 1.4 / TypeScript 7 toolchain, marks the developer-preview and safety status, and then gives the supported launch path, which clones and builds this fork. [The source-only launch Agent Note](2026-09-01-source-only-fork-launch-path.md) owns the removal of the npm launch path this section once listed. Toolchain rationale lives in [the bun package-manager Agent Note](2026-08-29-bun-package-manager.md) and [the TypeScript 7 Agent Note](2026-08-29-typescript-7-compiler.md); contributor setup lives in the [development guide](../../../../docs/development.md).

The launch path starts the Web UI through the `dsh` profile entry point, building the checkout before it runs `bun run dsh web`. Detailed ACP, TUI, SDK, capability, and package guidance stays in the user guide, architecture documentation, and package map instead of being repeated on the landing page.
