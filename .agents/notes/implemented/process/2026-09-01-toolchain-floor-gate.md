...
A coordinated downgrade of any of the seven toolchains, the engines string, or the bun pin now fails `ci-static`. The gate costs a maintenance edit whenever a floor is deliberately raised — and that edit is the reviewable act of saying which toolchain version CI now assumes. The first live run surfaced a stale tree (`packages/util/diagnostic-text/lib/index.js` missing, deadlocking the root tsdown config), bootstrapped by copying the tsc-emitted `lib/types/{index,invariant}.js` to `lib/`; `bun run build:lib:host` regenerates those files itself.
