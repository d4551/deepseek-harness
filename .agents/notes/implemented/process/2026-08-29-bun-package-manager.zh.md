...
- **仍写着 `pnpm` 的部分，用计数而非归类来说明。** 174 份已归档 Agent Note（共 214 个文件），按归档策略冻结。3 份已实现 Agent Note 把它作为历史而非指令写出：一次在旧工具链下测得的缓存耗时、Landlock 工作区对其合并掉的独立 workspace 的记述，以及被移除的 repository 插件对其删除内容的记述。3 份以 pnpm 本身为主题的 Agent Note——取代 Yarn 的决策、`pnpm/action-setup` 供给、按 runner 的安装隔离——已归档并从本文交叉链接，而非就地改写：被取代的决策不是过时的命令。其余已实现 Agent Note 全部完成转换。4 处 client 测试夹具中的示例字符串，描述的是产品用户在自己项目里运行过的命令。2 处注释陈述的是 corepack 为哪些工具提供 shim、以及 Stryker 的 `packageManager` 接受哪些取值——这些是关于那些工具的事实，而非关于本仓库。没有任何 vendored 文件提到它——本清单的早期版本曾声称存在一条覆盖零文件的 Vendoring Policy 豁免。4 份记录因其主题而写出它：本文、[审计循环记录](../../../audit-loop.md)、升级基线，以及下文的 gate。
...
`pnpm/action-setup` 接受按 job 区分的 `dest`，CI 工作流据此确保自托管 runner 上的并发 job 永不争抢同一份 pnpm 安装（见 [runner 隔离 Agent Note](../../archived/bug-fix/2026-07-29-pnpm-setup-runner-isolation.md)）。`oven-sh/setup-bun` 无条件安装到 `~/.bun/bin` 且不暴露等价入参，因此该隔离确实不可用。替代不变式是每个 `setup-bun` 步骤都解析出**同一个**版本：`scripts/ci-workflow.spec.ts` 断言每一处都传入 `bun-version-file: package.json` 且没有浮动的 `bun-version`，于是并发 job 安装的是完全相同的字节，而不是相互竞争的不同版本。
