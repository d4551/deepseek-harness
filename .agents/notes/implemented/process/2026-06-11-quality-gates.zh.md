...
本记录中的钩子/CI 对称设计已由[快速本地 Git 钩子](2026-07-22-fast-local-git-hooks.zh.md)取代；CI 仍是执行完整检查的路径。[完整源码质量门禁](../testing/2026-09-16-complete-source-quality-gates.zh.md)取代本记录对覆盖率指令的许可。
...
- [Oxlint](2026-07-29-oxlint-linter.zh.md) 配合类型感知的 TypeScript 规则以及 @stylistic 兼容插件，强制执行统一代码风格；vendor 代码排除在外。文件内重复逻辑检查已移交[语法级重复检测门](../testing/2026-08-31-syntax-duplication-gate.zh.md)。
- jscpd 检测包的生产 TypeScript 代码与仓库脚本中的跨文件克隆；窄范围的源码区间例外用于记录有意为之的并行实现。
- `packages/*/*/src` 下按文件 100% 覆盖率（V8），包括子进程和工作线程执行。覆盖率指令无论是否附有解释都会使源码审计失败。
...
- 100% 覆盖率的压力可能催生无断言的测试；[变异测试](../testing/2026-06-11-mutation-testing.zh.md)检查断言能否检测行为变化。
