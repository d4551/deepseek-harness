...
An unused disable directive also fails the gate: `reportUnusedDisableDirectives` is `error`, and the executable contract in `scripts/oxlint-contract.spec.ts` pins the failing exit status.
