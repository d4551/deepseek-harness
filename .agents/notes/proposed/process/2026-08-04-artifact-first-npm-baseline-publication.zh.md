...
安装使用本次发布选择的客户端行为。注册表上传必须使用 `npm` CLI（命令行界面），以满足私有注册表只接受 npm 客户端的策略；构建编排仍可使用 bun。tarball 测试不得先把这些包发布到真实注册表，也不得在测试后重新 pack。
...
全量 pack、安装和启动会增加 CI 时间与工作流产物体积。实现应缓存外部依赖和 bun install cache，但不得缓存或复用目标包的已安装 workspace 输出；并行执行安全的消费方 probe 可以降低时延。
