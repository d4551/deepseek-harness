...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' * `bun run gen-client-catalog` to regenerate (freshness-gated by',
    ' * `bun run verify-client-catalog` in doc-sync).',
    ' *',
...
export function main(): void {
  ...
  if (process.argv.includes('--check')) {
    ...
    console.error(`gen-client-catalog: stale — ${OUT}. Run \`bun run gen-client-catalog\` and commit the result.`)
    process.exit(1)
  }
