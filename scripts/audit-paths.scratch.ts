...
/** Export target: a path string or a conditional object the checks only stringify. */
type ExportTarget = string | object
...
const config = JSON.parse(stripped) as { compilerOptions: { paths: Record<string, string[]> } }
...
for (const m of raw.matchAll(keyRe)) {
  ...
  if (k !== undefined) seen.set(k, (seen.get(k) ?? 0) + 1)
}
...
const specRe = /(?:from\s*|import\s*\()\s*['"](@deepseek-ai\/dsh-[^'"]+?)['"]/g
const sideRe = /import\s+['"](@deepseek-ai\/dsh-[^'"]+?)['"]/g
const used = new Map<string, { count: number; examples: string[] }>()
for (const f of files) {
  ...
  for (const m of text.matchAll(specRe)) {
    const s = m[1]
    if (s !== undefined) local.add(s.split('?')[0] ?? s)
  }
  ...
  for (const s of local) {
    ...
    rec.count += 1
    if (rec.examples.length < 3) rec.examples.push(f.slice(ROOT.length + 1))
    used.set(s, rec)
  }
}

const bareTargets = new Map<string, string>()
for (const [key, targets] of Object.entries(paths)) {
  if (!key.startsWith(DSH) || key.slice(DSH.length).includes('/')) continue
  const t = targets.find(t2 => !t2.includes('*'))
  if (t !== undefined) bareTargets.set(key, resolve(ROOT, t))
}

const manifestCache = new Map<string, ScratchManifest>()
function manifestFor(spec: string): ScratchManifest | null {
  const base = bareTargets.get(spec)
  if (base === undefined) return null
  const pkgDir = join(base, '..')
  const mp = join(pkgDir, 'package.json')
  if (!existsSync(mp)) return null
  const cached = manifestCache.get(mp)
  if (cached !== undefined) return cached
  const parsed = JSON.parse(readFileSync(mp, 'utf8')) as ScratchManifest
