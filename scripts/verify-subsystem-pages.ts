...
export const GROUPS_WITHOUT_SUBSYSTEM_PAGE: Readonly<Record<string, string>> = {
  ...
  experimental: 'Private prototypes excluded from official releases; no supported subsystem contract exists to document.',
...
function main(): void {
  ...
  if (audit.violations.length > 0) {
    process.stderr.write('verify-subsystem-pages: package-group documentation violations found:\n')
    for (const violation of audit.violations) process.stderr.write(`  ${violation}\n`)
    process.exit(1)
  }
