...
export const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  ...
  'packages/util/thrown': 'The package is a type-only union erased at compile time.',
...
export const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-projection': { kind: 'none', reason: 'Browser-side folds over already-logged Session data; registers nothing model-facing.' },
  ...
  'packages/client/ui-workspace-roots': { kind: 'indirect', reason: 'The panel routes session.setWorkspaceRoots; the workspace/roots event it appends owns the model-visible effect through the search, language-server, instruction, and sandbox consumers that fold it.' },
