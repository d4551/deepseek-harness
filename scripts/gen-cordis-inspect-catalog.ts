...
function main(): void {
  ...
  const source = projector.renderRuntimeApi(clientModel(model))
    ...
    .replaceAll('bun run gen-cordis-api', 'bun run gen-cordis-inspect-catalog')
    .replaceAll('bun run verify-cordis-api', 'bun run verify-cordis-inspect-catalog')
  ...
  if (process.argv.includes('--check')) {
    if (!existsSync(destination) || readFileSync(destination, 'utf8') !== source) {
      throw new Error(`gen-cordis-inspect-catalog: ${CLIENT_OUT} is stale; run bun run gen-cordis-inspect-catalog`)
    }
