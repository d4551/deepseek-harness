...
import { DEPENDENCY_KINDS, localPackageDeclarations, type Manifest } from './npm-notice-manifests.ts'
...
function workspaceLinkedManifest(name: string): VirtualManifest | undefined {
  ...
  for (const [path, manifest] of loadWorkspaceManifests().manifests) {
    if (!DEPENDENCY_KINDS.some(kind => name in (manifest[kind] ?? {}))) continue
...
export function render(): string {
  ...
  const local = localPackageDeclarations(loadWorkspaceManifests().manifests)
  ...
  const localLines = local.map(({ name, manifest, kind, path }) => {
    if (!existsSync(resolve(root, path))) throw new Error(`${manifest}: local package ${path} is missing`)
    return `| \`${name}\` | [\`${manifest}\`](${manifest}) — \`${kind}\` | [\`${path}\`](${path}) |`
  })
