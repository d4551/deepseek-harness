...
# are snapshotted into a scratch directory, and the install and gates run there
# against the shared bun cache with the Wine-specific overrides (hoisted layout,
# win32-x64 platform packages) passed on the install command line. The Wine
# prefix and the checksum-verified Windows Node zip persist in
# .cache/wine-windows/ so reruns skip provisioning.
#
...
command -v bun > /dev/null || missing+=('bun (https://bun.com/docs/installation)')
if (( ${#missing[@]} > 0 )); then
...
snapshot_and_install() {
  ...
  # existence filter drops paths staged as deleted. The Wine-specific overrides
  # go on the install command line so the SNAPSHOT's bunfig.toml is untouched:
  # hoisted because Windows Node under Wine does not realpath the isolated
  # layout's symlinks, and win32/x64 so the Windows esbuild/rolldown/rollup
  # binaries materialize. Neither is recorded in the lockfile, so
  # --frozen-lockfile stays valid; --ignore-scripts skips host lifecycle
  # scripts no gate loads.
  ...
  (cd "$scratch/tree" && bun install --frozen-lockfile --ignore-scripts \
    --linker=hoisted --os win32 --cpu x64 > "$scratch/logs/install.log" 2>&1) && return 0
