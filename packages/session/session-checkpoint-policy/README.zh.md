...
三个屏障会被检查点化。直接或已准备的模型流首次迭代时，会先刷新活动会话的请求前缀，再构造流式中间件。无论监听器以何种顺序注册，回放都无法绕过此检查点。顶层工具调用在工具正文运行前被刷新，因此已记录调用在任何外部副作用前已持久；嵌套工具分派复用外层调用的检查点。在每个 `agent/pre-step` 边界，前一步骤提交的一切——其响应与有序工具结果——在派生下一个请求前被刷新。
...
模型就绪过程等待所有监听器结算。检查点失败会保留在抛出的 `AggregateError` 中，且不消耗回放条目。就绪前取消会返回 `ABORTED` 结束分片，而不启动检查点。就绪期间取消会等待已接受的工作：检查点失败仍会抛出；就绪成功则返回 `ABORTED` 结束分片而不分派。没有活动会话的请求不会触发会话检查点。
...
该插件参与强制并行的 `llm/request-ready` 事件，在预执行策略与防护之后包装 `tools/execute`，并监听 `agent/pre-step`。会话存储的 flush 是共享持久性屏障；并发工具检查点经它串行化，不会产生重复序列号。流式中间件只能在模型就绪成功后路由或回答请求。
