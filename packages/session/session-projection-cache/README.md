...
At startup, the domain validates every checkpoint against `checkpointRecord`. A schema-invalid record is durably deleted because the `sessions` table declares `rebuildable: true`; the warning names the domain, table, and session key. Valid neighboring records remain available, and session logs and workspace records are untouched. Deletion failures reject startup. A consumer rebuilds a missing checkpoint by supplying the complete authoritative log to `coldSnapshot`; if the process stops before that rebuild, the next startup still sees an absent checkpoint.
