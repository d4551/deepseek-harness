...
启动时，领域根据 `checkpointRecord` 校验每个检查点。`sessions` 表声明了 `rebuildable: true`，因此不符合 schema 的记录会被持久删除；警告会指明领域、表与会话键。相邻的有效记录继续可用，会话日志与工作区记录保持不变。删除失败会拒绝启动。消费方向 `coldSnapshot` 提供完整的权威日志以重建缺失的检查点；若进程在重建前停止，下次启动仍会看到该检查点缺失。
