...
`scrubSessionSnapshot` is the one committed-fixture projection that both comparison and write-back run, so `normalizeSessionSnapshot` and a refresh emit the same bytes. It keeps the session header line and every event payload, expands storage encodings a fixture never carries — range-encoded `sourceEventSeqs` and chunk runs a persistence flush boundary split — and omits ordinary `seq`/`time` and packed-row `seq0`/`time0` envelopes. Replay synthesizes the envelopes in memory, while runtime persistence continues to write complete logs. That output is already the canonical packed layout `scripts/session-fixture-layout.ts` enforces, so the [temporary repository migrator](../../../scripts/migrate-packed-session-fixtures.ts) (`bun run migrate:packed-session-fixtures`) converts only fixtures written before this projection, and its [removal proposal](../../../.agents/notes/proposed/process/2026-07-26-remove-packed-session-fixture-migrator.md) owns its deletion.
