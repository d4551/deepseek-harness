...
import { waitForPersistence } from './persistence-deadline.ts'
...
async function waitForPersistedTurnStart(
  ...
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist ${detail} within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedTurnEnd(
  ...
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist turn/end within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedChildTurnEnd(
  ...
  }, new Error(`snapshot-harness: subagent child #${child} did not persist closed turn ${minimumTurn} within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedGoalPhase(
  ...
  await waitForPersistence(async () => {
    ...
    return matched
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist goal phase "${phase}" within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedInboxMessage(
  ...
  await waitForPersistence(async () => {
    ...
    return matched
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist expected inbox message within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedTitleAfterTurnEnd(
  ...
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist session/title after turn/end within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForPersistedEventAfterTurnEnd(
  ...
  }, new Error(`snapshot-harness: session "${sessionId}" did not persist ${type} after turn/end within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
...
async function waitForWorkspaceFile(
  ...
  await waitForPersistence(() => existsSync(target),
    new Error(`snapshot-harness: workspace file "${path}" did not appear within ${timeoutMs}ms`), timeoutMs, WAIT_POLL_INTERVAL_MS)
}
