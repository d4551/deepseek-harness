...
`web/` 组让 Agent 通过 `web_search` 和 `web_fetch` 搜索网络与抓取网页。本地 Playwright Chromium 提供无需付费搜索 API 的浏览器搜索，并抓取渲染页面。Exa、Perplexity 和 DeepSeek 提供其他搜索后端；HTTP 提供匿名字节抓取。共享的 `ctx.web` 服务拥有提供方选择、取消和错误。提供方拥有目标检查和资源上限。
...
| [`web-fetch-playwright/`](web-fetch-playwright/README.zh.md) | 通过本地 Chromium 搜索 Bing 并抓取渲染页面 | 注册到 `ctx.web` |
