 ...
 * `./policy` and `./network` subpaths declared in `package.json` need their own
 * bundles or a packed install resolves them to missing files.
 */
export default defineConfig([
  ...
  {
    entry: ['lib/types/policy.js'], outDir: 'lib', format: ['esm'], platform: 'node', target: 'es2024',
    fixedExtension: false, outputOptions: { codeSplitting: false }, dts: false, clean: false,
  },
  {
    entry: ['lib/types/network.js'], outDir: 'lib', format: ['esm'], platform: 'node', target: 'es2024',
    fixedExtension: false, outputOptions: { codeSplitting: false }, dts: false, clean: false,
  },
])
