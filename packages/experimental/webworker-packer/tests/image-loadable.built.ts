 ...
 *
 * Built-artifact lane: the image materializes each packed package's emitted
 * `lib/`, so this file lives under the `*.built.ts` include of
 * vitest.built.config.ts and its gate runs after `build`. Overlay packing that
 * needs no build output stays in tests/vfs-overlay.spec.ts.
 *
...
import type { TimeoutReason } from '@deepseek-ai/dsh-timeout'
...
describe('packed image', () => {
  ...
  it('records the lowering pass in the manifest and rewrote what it visited', () => {
    const result = packed()
  ...
  it('mounts and requires through the real loader, which carries no transform', async () => {
    ...
    const required = loader.requireFrom(`${DEFAULT_ROOT}/workspace`)(SUBJECT) as {
      timeoutOf(x: AbortSignal, code?: string): TimeoutReason | undefined
    }
  ...
  it('keeps third-party runtime JavaScript published under src', async () => {
    ...
    await loader.precompile()
    const webserver = loader.requireFrom(`${DEFAULT_ROOT}/workspace`)(WEB_SERVER) as {
      WebServer: new (ctx: object, config: object) => object
    }
  ...
  it('prepares the unchanged plugin-package inventory through Worker createRequire paths', async () => {
    ...
    const inventory = loader.requireFrom(`${DEFAULT_ROOT}/workspace`)(PLUGIN_INVENTORY) as {
      apply(ctx: object, config: object): void
    }
  ...
  it('refuses a body the packer did not lower, naming the image', async () => {
    ...
    await expect(loader.precompile())
      .rejects.toThrow(/still carries module syntax, so the image was not lowered by the packer/)
  })
})
