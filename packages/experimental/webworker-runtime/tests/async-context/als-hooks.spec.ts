...
// 3. Rejection still closes the boundary, and the caller still sees the rejection.
{
  ...
  const message = await failing.then(() => 'resolved', (error: unknown) => error instanceof Error ? error.message : String(error))
...
//     a stand-in for an absent fulfilled handler.
{
  ...
  const outcome = await als.run('slots', () => Promise
    ...
    .then(undefined, (error: unknown) => `caught:${error instanceof Error ? error.message : String(error)}`))
