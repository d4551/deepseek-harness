...
本包提供附件的本地存储与图片处理后端：源图经过校验、方向修正、元数据与色彩配置移除，并规范化为 8-bit sRGB/sRGBA 后保存在 `DSH_HOME` 下；路由专用请求版本另行派生并缓存。随附的 `dsh` 组合使用的就是它，因此持久图片附件无需配置即可工作。相同规范化图片只存一份，引用元数据与请求变体匹配的并发读取共享工作，即使后来收紧准入限制，已存图片仍然可读。存储仅限本机——其他主机无法读取这些图片——对象也永远不会自动删除。
...
并发的请求读取仅在附件引用的全部元数据与请求变体都匹配时共享工作。每个结果都保留其引用的名称与原始尺寸。引用声明的字节数、媒体类型、宽度或高度若与已存图片不符，该读取以 `ATTACHMENT_CORRUPT` 拒绝，不受使用同一图片有效引用的并发读取影响。
...
请求取消会保留调用方的原始原因。一个调用方取消时，其他等待方仍继续运行；最后一个调用方取消时，会等待共享操作与缓存清理结束。中止事件监听器无法阻止提供方观察取消。排队请求会立即退出压缩队列，正在运行的编码器会在候选之间检查取消。缺失或无效的缓存图片字节会重新生成；其他缓存文件系统错误会直接传播，不会被当作未命中。

准入取消会立即移除排队中的压缩工作。正在运行的原生操作会保持占用并发槽位直到结束，批次会等待全部准备工作结束后才返回失败。原子发布开始前的取消会阻止该图片对象发布；发布开始后则会完成持久化工作。已取消批次中较早提交的成员仍保留在存储中。`read_image` 工具会把执行信号传入准入操作。

对象存放在 `<DSH_HOME>/attachments/v1/objects/<sha256-prefix>/<sha256>`；相同字节会去重为同一个对象和同一个 `sha256:` 标识符。POSIX 首次写入前，进程会把 home 的每个祖先目录逐级同步到文件系统根目录，因此绝不会把另一个进程已创建但尚未同步的目录误认为安全边界。写入过程把字节暂存到 `v1/tmp`，并同步暂存文件。POSIX 以原子且排他的硬链接发布，并同步发布目录；Windows 使用[共享 write-through 辅助函数](../../util/atomic-write/src/win32.ts)创建缺失目录并移动新对象，不替换已有目标。保存成功后，已报告的引用即持久。
...
请求版本位于 `<DSH_HOME>/attachments/v1/request-images/`。`readImageRequest` 在不放大的前提下缩放到路由像素预算，再通过相同的 alpha 路由与质量阶梯应用独立编码字节目标。缓存身份包含附件 id、变换版本、预算与固定编码参数；缓存字节会先探测格式、8-bit sRGB/sRGBA、尺寸与 alpha 信息，不匹配时重新生成。引用元数据与请求变体匹配的调用方共享一次变换与缓存写入，且只在没有等待方时由取消停止共享工作。`imageHostPath` 派生规范化对象的宿主路径，挂载的文件系统可以把该路径映射进执行世界，而不会写入持久历史。

### 源码地图

| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：`LocalAttachmentStore`、`Config` schema、默认值、共享请求工作 |
...
| [`src/shared-request.ts`](src/shared-request.ts) | 共享变换所有权、调用方独立取消与最终结束等待 |
