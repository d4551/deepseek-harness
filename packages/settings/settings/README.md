...
`settings/registry-updated (ns)` announces a namespace registration or its completed release. Consumers re-read `describe()` to obtain the current directory. Registering and releasing leave stored sections, revisions, and capability readiness unchanged; the release notification follows the owned watchers' completion.
