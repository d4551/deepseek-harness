---
description: "零依赖工具家族的包映射：原子文件写入、品牌化 id、拒绝值联合类型、harness 主目录路径、启动环境、原生命令、输出保留与超时。"
kind: "package-group"
---
...
`util/` 组为能力包提供共享的机制原语，避免重复实现。它涵盖原子写入、品牌化 id、拒绝值联合类型、UUID、Harness home 路径、启动环境、原生命令、输出保留和超时处理。这里的每个包都是库：它不注册服务或事件，业务语义仍由消费它的能力负责。
...
| [`thrown/`](thrown/README.zh.md) | 为观察 throw 或 Promise 拒绝的处理方提供仅编译期的拒绝值联合类型 |
