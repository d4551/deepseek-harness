...
Codex 子级会在一个全新的临时线程中，以单个轮次接收这些独立文本块。它的工作区就是父会话的工作区：会话 cwd，加上该会话的其他工作区根目录（在 `thread/start` 上声明为该线程的 `sandbox_workspace_write.writable_roots`）。无法识别的 `config` 键会被静默忽略而不报错，因此声明的根目录集合会与 `thread/start` 返回的策略比对确认；当一个 `workspaceWrite` 线程并未采纳它时，运行会被拒绝；所选提供方实例会固定已配置的模型、环境、非交互审批策略与沙箱模式，而省略的模型及其余产品设置来自 Codex 原生配置。可执行版本来自 Bundle 锁定的平台载荷。
