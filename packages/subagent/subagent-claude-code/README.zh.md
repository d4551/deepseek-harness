...
Claude Code 子级会在一个全新的 SDK query 中接收独立文本任务。它的工作区就是父会话的工作区：会话 cwd，加上该会话的其他工作区根目录（作为 SDK 的 `additionalDirectories` 传入）；所选提供方实例会固定已配置的模型、环境与非交互权限模式，而省略的模型及其余产品设置来自 Claude 原生配置。可执行版本来自 Bundle 锁定的 SDK 平台载荷。
