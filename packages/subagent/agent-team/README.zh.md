...
挂载工作区注册表后，roster 还会以 `peer` 角色显示注册在同一工作区中的实时会话 Lead。它们的 `session:<id>` 名称可作为消息目标。发现过程排除已归档会话，并要求经过验证的注册表成员关系；仅共享目录不会授予访问权限。quiet 消息保留持久接收记录，不会启动空闲 peer。每个会话拥有自己的任务板，因此 peer 通过消息协调职责。
