---
description: "在一个会话中运行一个小型具名 agent 团队：成员之间的持久消息与共享任务板，供组合 Team 插件的部署方阅读。"
...
`@deepseek-ai/dsh-agent-team` 把一个编码会话变成一个小型工作团队：会话中的 agent 成为 Lead，创建具名 teammate 处理委派的工作，与它们交换持久消息，并在公共任务板上跟踪共享任务。消息与任务状态能挺过崩溃、reload 与中断，因此离线的 teammate 会在恢复后收到排队的消息。它本身不提供任何工具——请挂载兄弟包 `@deepseek-ai/dsh-tool-agent-team`，让模型能够创建 teammate、给它们发消息并使用任务板。它以 prerelease 形式发布、不承诺稳定性，并且需要持久会话存储才能激活。
...
挂载工作区注册表后，roster 还会以 `peer` 角色显示注册在同一工作区中的实时会话 Lead。它们的 `session:<id>` 名称可作为消息目标。发现过程排除已归档会话，并要求经过验证的注册表成员关系；仅共享目录不会授予访问权限。quiet 消息保留持久接收记录，不会启动空闲 peer。每个会话拥有自己的任务板，并通过消息协调职责。同一 host 中的实时 peer 任务板参与工作区写入范围检查；冲突会标明所属会话与任务。
...
写入范围会阻止本地任务板或同一注册工作区内实时 peer 任务板上的重叠任务并发运行。有冲突的认领、重新分配或范围变更会被拒绝；自动任务选择会等待重叠工作完成。这些检查约束任务归属，不限制文件系统写入。已删除任务保留在历史中，但从活动列表中消失。
...
- [实验包决策](../../../.agents/notes/implemented/architecture/2026-08-18-experimental-agent-teams-packages.zh.md)——最初的位置与依赖隔离决策。
