...
describe('Team shared task DAG', () => {
  ...
  it('reports an empty and a fully blocked board as no ready task rather than a failure', async () => {
    ...
    const blocked = await ctx.agentTeams.createTask(lead, {
      subject: 'blocked', description: 'runs second', blockedBy: [blocker.id],
    })
    ...
    if (first.outcome !== 'claimed') throw new Error('expected the blocker to be claimed')
    await ctx.agentTeams.updateTask(lead, {
      taskId: blocker.id, expectedRevision: first.task.revision, action: 'complete',
    })
    await expect(ctx.agentTeams.claimNextReadyTask(lead))
    ...
    await expect(ctx.agentTeams.claimNextReadyTask({ id: SessionId('outsider') } as unknown as Agent))
      .rejects.toMatchObject({ code: 'TEAM_NOT_MEMBER' })
  })

  it('refuses a ready task whose write scopes overlap in-progress work and names it', async () => {
    const { ctx, lead } = await setup(['hang'])
    const member = await spawn(ctx, lead, 'scoped')
    const worker = await waitRunning(ctx, member.member.id)

    const broad = await ctx.agentTeams.createTask(lead, {
      subject: 'broad', description: 'writes the whole tree', writeScopes: ['src'],
    })
    ...
    const disjoint = await ctx.agentTeams.createTask(lead, {
      subject: 'disjoint', description: 'writes elsewhere', writeScopes: ['docs'],
    })

    const taken = await ctx.agentTeams.claimNextReadyTask(lead)
    expect(taken).toMatchObject({ outcome: 'claimed', task: { id: broad.id } })
    // The overlapping task is skipped, and the disjoint one behind it is taken.
    await expect(ctx.agentTeams.claimNextReadyTask(worker))
      .resolves.toMatchObject({ outcome: 'claimed', task: { id: disjoint.id } })
    await expect(ctx.agentTeams.claimNextReadyTask(worker))
      .resolves.toEqual({ outcome: 'none', reason: 'write-scope-conflict', deferred: [overlapping.id] })

    if (taken.outcome !== 'claimed') throw new Error('expected the broad task to be claimed')
    await ctx.agentTeams.updateTask(lead, {
      taskId: broad.id, expectedRevision: taken.task.revision, action: 'complete',
    })
    await expect(ctx.agentTeams.claimNextReadyTask(worker))
  ...
  it('hands concurrent claimants disjoint tasks and never the same task twice', async () => {
    const { ctx, lead } = await setup(['hang', 'hang', 'hang'])
    const workers: Agent[] = [lead]
    for (const name of ['w1', 'w2', 'w3']) {
      const member = await spawn(ctx, lead, name)
      workers.push(await waitRunning(ctx, member.member.id))
    }

    const created: TeamTaskId[] = []
    for (let index = 0; index < 6; index += 1) {
      const task = await ctx.agentTeams.createTask(lead, {
        subject: `unit ${index}`,
        description: `independent unit ${index}`,
        writeScopes: [`src/unit-${index}`],
      })
      created.push(task.id)
    }

    // Every member claims twice with no ordering between the calls.
    const outcomes = await Promise.all(
      [...workers, ...workers].map(member => ctx.agentTeams.claimNextReadyTask(member)),
    )
    const claimed = outcomes.flatMap(outcome => outcome.outcome === 'claimed' ? [outcome.task] : [])
    expect(claimed).toHaveLength(6)
    expect(new Set(claimed.map(task => task.id)).size).toBe(6)
    expect([...claimed.map(task => task.id)].sort()).toEqual([...created].sort())
    for (const task of claimed) expect(task.status).toBe('in_progress')

    const board = ctx.agentTeams.listTasks(lead)
    expect(board.every(task => task.status === 'in_progress')).toBe(true)
    // Every task has exactly one durable owner and its revision advanced once.
    expect(durable(lead).tasks.map(task => task.revision)).toEqual([2, 2, 2, 2, 2, 2])
