...
/** Every Team tool with arguments its own schema accepts, split by whether it mutates. */
const READ_ONLY_CALLS: [string, Record<string, unknown>][] = [
  ['list_agents', {}],
  ['team_task_list', {}],
  ['team_task_get', { task_id: 'task-1' }],
]
const MUTATING_CALLS: [string, Record<string, unknown>][] = [
  ['spawn_teammate', { name: 'worker', description: 'work', prompt: 'do it' }],
  ['send_message', { target: 'lead', message: 'note' }],
  ['followup_task', { target: 'lead', message: 'note' }],
  ['wait_agent', {}],
  ['interrupt_agent', { target: 'worker' }],
  ['team_task_create', { subject: 'task', description: 'details' }],
  ['team_task_claim_next', {}],
...
describe('dsh-tool-team', () => {
  ...
  it('declares board reads concurrency safe and every Team mutation exclusive', async () => {
    const { ctx, lead } = await setup(['hang'])
    ...
    for (const [name, args] of READ_ONLY_CALLS) {
      expect(mode(name, args), `${name} must be parallel`).toEqual({ kind: 'parallel' })
    }
    for (const [name, args] of MUTATING_CALLS) {
      expect(mode(name, args), `${name} must be exclusive`).toEqual({ kind: 'exclusive' })
    }
    // The reads are fail-closed too: arguments their own schema rejects fall back
    // to exclusive rather than running beside another call.
    expect(mode('team_task_get', {})).toEqual({ kind: 'exclusive' })
  })

  it('claims the next ready task, reports an empty board, and reports a scope conflict', async () => {
    const { ctx, lead } = await setup(['hang'])
    ...
    await execute(ctx, lead, 'team_task_create', {
      subject: 'overlapping', description: 'writes inside it', write_scopes: ['src/nested'],
    })

    const claimed = await execute(ctx, lead, 'team_task_claim_next', {})
    expect(claimed.isError).toBe(false)
    expect(JSON.parse(text(claimed))).toMatchObject({
      outcome: 'claimed',
      task: { id: 'task-1', status: 'in_progress', ownerName: 'lead', revision: 2 },
    })

    const conflicted = await execute(ctx, lead, 'team_task_claim_next', {})
    expect(conflicted.isError).toBe(false)
    expect(JSON.parse(text(conflicted)))
      .toEqual({ outcome: 'none', reason: 'write-scope-conflict', deferred: ['task-2'] })
  })

  it('gives swarm deployments pull-based guidance in place of the delegated policy', async () => {
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const storageRoot = mkdtempSync(join(tmpdir(), 'dsh-tool-team-swarm-'))
    roots.push(storageRoot)
    await ctx.plugin(JsonlSessionPersistence, { root: storageRoot })
    await ctx.plugin(TestSessionQuery)
    await ctx.plugin(AgentLoop, { agents: [] })
    ...
    await ctx.plugin(SubagentFork, { providerName: 'fork' })
    await ctx.plugin(TeamService)
    await ctx.plugin(toolTeam, { coordination: 'swarm' })
    ctx.llm.registerAdapter(['mock'], new MockAdapter(['hang']))
    const lead = ctx.agentLoop.create(SessionId('swarm-tool-lead'), { provider: 'mock', model: 'mock' })
