 ...
 * `bun run gen-cordis-inspect-catalog` to regenerate (freshness-gated by
 * `bun run verify-cordis-inspect-catalog` in doc-sync).
 *
...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'fork(opts: { sessionId: SessionId; atSeq?: number; increaseTitle?: boolean }): Promise<ClientResult<SessionId>>',
  ...
  {
    key: 'timer',
    summary: 'Browser timer Service whose pending work belongs to the calling Fiber.',
    description: 'Browser timer Service whose pending work belongs to the calling Fiber.',
    methods: [
      {
        signature: 'timeout(callback: () => void, delay: number): TimerDisposer',
      ...
      {
        signature: 'interval(callback: () => void, delay: number): TimerDisposer',
      ...
      },
      {
        ...
        returns: 'async iterator of ticks. Its `throw()` keeps an Error\'s identity; an omitted reason becomes an Error, and other values become a TypeError with the original cause. Disposing the calling Fiber rejects pending and subsequent `next()` calls.',
      },
      {
        signature: 'throttle<Args extends unknown[]>(callback: (...args: Args) => void, delay: number, noTrailing?: boolean): Scheduled<Args>',
        description: 'Return a throttled function whose timer is disposed with the calling Fiber.',
        parameters: [{ name: 'callback', description: 'Function to throttle.' }, { name: 'delay', description: 'Minimum interval between calls in milliseconds.' }, { name: 'noTrailing', description: 'Whether to omit a delayed trailing call.' }],
        returns: 'Throttled function with an early disposer.',
      },
      {
        signature: 'debounce<Args extends unknown[]>(callback: (...args: Args) => void, delay: number): Scheduled<Args>',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'TimerDisposer',
    declaration: 'export type TimerDisposer = () => void | Promise<void>;',
  },
  {
    name: 'Translate',
