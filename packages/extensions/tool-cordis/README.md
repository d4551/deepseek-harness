...
| [`src/api-catalog.ts`](src/api-catalog.ts) | Generated projection of the workspace's Cordis declarations (regenerated by `bun run gen-cordis-api`, gated by `verify-cordis-api`) |
