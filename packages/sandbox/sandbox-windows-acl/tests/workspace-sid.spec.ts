/**
 * workspaceWriteSid tests: the per-workspace-scope write identity is
 * deterministic (the same canonical root set always derives the same SID — the
 * property the cross-session grant reuse rests on), capability-shaped,
 * order-independent, distinct across scopes, and byte-sensitive (the canonical
 * path is the caller's contract; an alias spelling derives a second identity,
 * self-healing at the cost of one extra tree propagation).
 */
...
describe('workspaceWriteSid', () => {
  it('derives a stable capability-shaped SID per workspace root set', () => {
  ...
  it('identifies the SET, not the listing order, and separates a widened scope from its primary root alone', () => {
    const roots = ['C:\\Users\\agent\\repo', 'C:\\Users\\agent\\other']
    expect(workspaceWriteSid(roots)).toBe(workspaceWriteSid([...roots].reverse()))
    expect(workspaceWriteSid(roots)).not.toBe(workspaceWriteSid(['C:\\Users\\agent\\repo']))
  })
