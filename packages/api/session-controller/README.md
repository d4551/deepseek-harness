...
`SessionEventStream` binds a Gateway `RemoteJournalStream` to one ordinary or direct-subagent address. Each follow generation carries its opening snapshot; sequence gaps trigger a tail-page read. The stream publishes only contiguous `replace`, `prepend`, and `append` changes. Ordinary records cover `[event.seq, event.seq]`; packed rows cover `[event.seq, event.seq + memberCount - 1]`. A business, persistence, or unresolved continuity failure terminates the stream, while physical carrier loss selects automatic resumption. `SessionControlStream` opens each generation with a complete process-local baseline for queues, jobs, and projections.

The Client session manager retains a catalog-selected child's direct-parent address, so a detached worker's durable conversation remains reachable. Membership updates refresh the selected child's sibling catalog as well as open branches. Repeated unchanged summary events preserve the list snapshot; running-to-idle transitions still update completion reminders. Disposal cancels owned reads and waits for pending operations and resident sessions to settle.
