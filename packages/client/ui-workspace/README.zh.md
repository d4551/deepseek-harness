...
Session 行还支持范围选择，以便一次归档多个会话。点击一行即确定范围锚点；按住 Shift 点击下方某一行会选中两者之间渲染出的全部行，而由于计数依据是可见的阅读顺序，范围可以跨越 Workspace 分组。Ctrl/Cmd 点击可增删单行，普通点击或 Escape 撤销选择，在选择之外右击则把选择收窄到指针所在的那一行。选中两行及以上时，行菜单以 **Archive N sessions** 取代逐行操作项，并为每个选中的 Session 各提交一次归档。
