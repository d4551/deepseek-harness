...
折叠搜索是视图和添加操作旁的一枚区头按钮：激活后输入框会扩展并占据区头。非空白查询会以单一扁平结果列表替代任一浏览模式——不区分大小写的标题和 Workspace 子串匹配项会立即显示，经 250 ms 防抖的 Host 请求则会加入经过排序的当前对话内容匹配项及其摘要片段。每次新查询都会中止前一个请求；内容搜索失败时，元数据匹配项仍会显示，同时给出警告。列表最多显示 20 条结果，打开所选 Session 时不会清除查询。结果行与两个会话列表一样只保留一个 Tab 停靠位：方向键在其间移动，Home 与 End 跳到两端，Enter 打开获得焦点的结果。
...
每一行的菜单都响应三种手势：悬停时显现的 **...** 按钮，在该行任意位置右击——后者在指针处打开同一份菜单——以及平台自身的菜单键 Shift+F10 与 ContextMenu 键，它们在获得焦点的那一行处打开菜单并把焦点移入其中。临时的空白 **New Session** 行与 Ungrouped 分组头没有任何操作项，因此二者保留平台自带菜单。

右击和键盘菜单在指针移开后仍保持打开；Escape、外部按压或选择操作会将其关闭。在 macOS 上，按住 Control 点击行、其 **...** 或 **+** 都只请求该行菜单，不会再次关闭菜单或新建 Session。聚焦 **...** 后按 Enter 或空格会打开键盘菜单，关闭时会在尾部按钮消失前交还焦点。

两种行都支持范围选择，以便一次归档多个会话。点击一行即确定范围锚点——项目行在该次点击时仍会展开或折叠——按住 Shift 点击下方某一行则选中两者之间渲染出的一切；由于计数依据是可见的阅读顺序，范围可以跨越 Workspace 分组头及其下的分组。平台的增删修饰键——Apple 平台上是 Cmd，其他平台上是 Ctrl——可增删单行；Apple 主机把 Ctrl 点击用于次级菜单，行会把那次按压留给菜单。普通点击或 Escape 撤销选择，在选择之外右击则把选择收窄到指针所在的那一行。被选中的项目无论是否展开都代表它的 Session，屏幕上的每个成员行都会随之一同标记。折叠的项目，以及超出折叠行数上限的成员，会被触及却不被标记：**Archive N sessions** 的计数与状态区域是它们仅有的交代。选中两行及以上时，列表中每个行菜单都以 **Archive N sessions** 取代其逐行操作项，为触及的每个 Session 各提交一次归档；当一段只含空项目的范围触及不到任何 Session 时，该项处于不可用状态。

这一切键盘同样可达。每个列表在其所有行之间只保留一个 Tab 停靠位——折叠分组持有的 **Show N more** 行也是其中一行，而不是它们旁边的一个控件——因此一次 Tab 即可抵达浏览区域；方向键在其内部移动，Home 与 End 跳到两端，左右方向键操作项目的展开状态——展开折叠的分组、折叠展开的分组，并进入或退回到 Session 行所属的分组头。Enter 等同于普通点击；只要范围能触及该行，Space 就像增删修饰键点击那样增删获得焦点的那一行，范围触及不到时则激活该行；Shift 与方向键或 Space 组合带上范围，锚点仍在范围起始处；增删修饰键加 A 则一次取走范围能触及的每一行。键入可打印字符会把焦点移到下一个标签以它开头的行，重复同一个字符会在以它开头的各行间走动，一秒之内再键入一个字符则会细化这次搜索——无论侧边栏有多长都无需逐屏翻找即可到达。行内获得焦点时会显现与悬停相同的尾部操作项，因此完全不用指针也能到达 **...** 按钮；用键盘打开它的列表时，焦点会直接落在列表的第一行。
...
- **范围之外的行无法发起范围**：范围触及不到的行——Ungrouped 分组头、临时的空白草稿、折叠分组的 **Show N more** 行——在 Shift 下只移动焦点、不带走任何东西，因为它不在范围所跨越的那份计数之内。Shift *跨过*这样的行则一如既往地带走范围；而在计数之内的任意一行上，增删修饰键加 A 会取走整份计数。
- **Type-ahead 只读取浏览器以单个字符命名的按键**：输入法在合成文本时不会发出这样的按键，因此合成中文或日文的会话名不会搜索列表；方向键与 Home/End 则不受影响，仍能到达每一行。
