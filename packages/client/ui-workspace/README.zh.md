...
两种行都支持范围选择，以便一次归档多个会话。点击一行即确定范围锚点——项目行在该次点击时仍会展开或折叠——按住 Shift 点击下方某一行则选中两者之间渲染出的一切；由于计数依据是可见的阅读顺序，范围可以跨越 Workspace 分组头及其下的分组。Ctrl/Cmd 点击可增删单行，普通点击或 Escape 撤销选择，在选择之外右击则把选择收窄到指针所在的那一行。被选中的项目无论是否展开都代表它的 Session，其成员行会随之一同标记，因此标记始终如实显示批量操作将触及的范围。选中两行及以上时，列表中每个行菜单都以 **Archive N sessions** 取代其逐行操作项，为触及的每个 Session 各提交一次归档；当一段只含空项目的范围触及不到任何 Session 时，该项处于不可用状态。

这一切键盘同样可达。每个列表只保留一个 Tab 停靠位，因此进入浏览区域只需一次 Tab、离开也只需一次；方向键在其内部移动，Home 与 End 跳到两端，左右方向键操作项目的展开状态——展开折叠的分组、折叠展开的分组，并进入或退回到 Session 行所属的分组头。Enter 等同于普通点击，Space 像 Ctrl/Cmd 点击那样增删获得焦点的那一行，Shift 与方向键或 Space 组合则带上范围，锚点仍在范围起始处。行内获得焦点时会显现与悬停相同的尾部操作项，因此完全不用指针也能到达 **...** 按钮；用键盘打开它的列表时，焦点会直接落在列表的第一行。
