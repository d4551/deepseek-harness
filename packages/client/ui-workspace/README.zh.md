...
这一切键盘同样可达。每个列表只保留一个 Tab 停靠位，因此进入浏览区域只需一次 Tab、离开也只需一次；方向键在其内部移动，Home 与 End 跳到两端，左右方向键操作项目的展开状态——展开折叠的分组、折叠展开的分组，并进入或退回到 Session 行所属的分组头。Enter 等同于普通点击，Space 像 Ctrl/Cmd 点击那样增删获得焦点的那一行，Shift 与方向键或 Space 组合则带上范围，锚点仍在范围起始处。行内获得焦点时会显现与悬停相同的尾部操作项，因此完全不用指针也能到达 **...** 按钮。
