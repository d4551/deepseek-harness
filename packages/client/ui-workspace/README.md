...
Rows also take a range selection for archiving several Sessions at once, and the range spans both row kinds. Clicking a row anchors the range — a project row still folds or unfolds on that click — and shift-clicking a lower row selects everything rendered between the two; because the account is the visible reading order, a range crosses Workspace headers and the groups under them. Ctrl/Cmd-click adds or removes single rows, a plain click or Escape withdraws the selection, and a right-click outside it narrows the selection to the row under the pointer. A selected project stands for its Sessions whether or not it is unfolded, and those member rows mark with it, so the marking always shows exactly what a bulk action would reach. With two or more rows selected, every row menu in the list replaces its per-row verbs with **Archive N sessions**, which commits one archive per reached Session and is inert when a range of empty projects reaches none.

The keyboard reaches all of it. Each list keeps one tab stop, so the browsing region costs one Tab to enter and one to leave; the arrows move inside it, Home and End jump to either end, and the horizontal arrows work a project's disclosure — opening a folded group, folding an open one, and stepping into or back out to the header a Session row sits under. Enter does what a plain click does, Space adds or removes the focused row the way Ctrl/Cmd-click does, and Shift with an arrow or with Space takes the range along, anchored where the range began. Focus inside a row reveals the same trailing verbs hover does, so the **...** button is reachable without a pointer at all, and opening its list from the keyboard hands the operator its first row.
