...
Every row menu answers three gestures: the hover-revealed **...** button, a right-click anywhere on the row, which opens the same list under the pointer, and the platform's own menu keys — Shift+F10 and the ContextMenu key — which open it against the focused row and move the focus into it. The provisional blank **New Session** and the Ungrouped bucket header have no verbs, so both keep the platform menu.

Right-click and keyboard menus stay open while the pointer moves away; Escape, an outside press, or choosing an action closes them. On macOS, Control-click on a row, its **...**, or its **+** requests the row menu without toggling it shut or creating a Session. Enter and Space on **...** open the keyboard menu, and dismissal returns focus before the trailing buttons can disappear.

Rows also take a range selection for archiving several Sessions at once, and the range spans both row kinds. Clicking a row anchors the range — a project row still folds or unfolds on that click — and shift-clicking a lower row selects everything rendered between the two; because the account is the visible reading order, a range crosses Workspace headers and the groups under them. The platform's additive modifier — Cmd on Apple platforms, Ctrl elsewhere — adds or removes single rows; Apple hosts spend Ctrl+click on the secondary menu and the row leaves that press to it. A plain click or Escape withdraws the selection, and a right-click outside it narrows the selection to the row under the pointer. A selected project stands for its Sessions whether or not it is unfolded, and every member row that is on screen marks with it. A folded project, and any member past the collapsed row limit, is reached without being marked: the **Archive N sessions** count and the status region are the only account of those. With two or more rows selected, every row menu in the list replaces its per-row verbs with **Archive N sessions**, which commits one archive per reached Session and is inert when a range of empty projects reaches none.

The keyboard reaches all of it. Each list keeps one tab stop across all of its rows — the **Show N more** row a folded group holds is one of them, not a control beside them — so one Tab reaches the browsing region; the arrows move inside it, Home and End jump to either end, and the horizontal arrows work a project's disclosure — opening a folded group, folding an open one, and stepping into or back out to the header a Session row sits under. Enter does what a plain click does, Space adds or removes the focused row the way an additive click does wherever a range reaches it — and activates the row where none does — Shift with an arrow or with Space takes the range along, anchored where the range began, and the additive modifier with A takes every row the range reaches at once. Typing a printable character moves the focus to the next row whose label starts with it, repeating one character walks the rows that begin with it, and a further character within a second refines the search — a sidebar of any length is reachable without paging through it. Focus inside a row reveals the same trailing verbs hover does, so the **...** button is reachable without a pointer at all, and opening its list from the keyboard hands the operator its first row.
