...
Settings 与 Agent Team 使用共享的 Modal 基础组件和主题字体规范。分区控件在窄屏上自动换行，内容滚动时标题栏和 Settings 导航保持可见，关闭弹窗之前键盘焦点始终留在弹窗内。设置菜单保留在所属弹窗内；选择选项后 Settings 保持打开，按 Escape 时先关闭已打开的菜单，再关闭弹窗。
