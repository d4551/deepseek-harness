...
import { TeamAction, actions, props, remoteFailure, view, type TeamActionInjected } from './team-fixtures.client.ts'
...
it('replaces initial loading with the failed discovery result', async () => {
  ...
  expect((await screen.findByRole('alert')).textContent).toBe('Discovery disconnected')
...
it('retains tracked work during a rejected refresh and recovers on retry', async () => {
  const load = vi.fn<TeamActionInjected['load']>()
  ...
  expect((await screen.findByRole('alert')).textContent).toBe('refresh disconnected')
...
it('does not publish a navigation rejection into another conversation', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: /Agent Team/u }))
  fireEvent.click(await screen.findByRole('button', { name: `${zh.open}: worker` }))
...
it('shows a plain Thrown discovery refusal without an Error prefix', async () => {
  render(<TeamAction {...props(actions({ load: () => Promise.reject('plain refusal') }))} />)
  fireEvent.click(screen.getByRole('button', { name: /Agent Team/u }))
