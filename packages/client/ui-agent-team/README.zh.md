...
实时订阅会在 view 读取尚未完成时合并活动通知。只有最新一次刷新可以发布结果或错误。overview 读取失败后仍会继续观察活动，因此下一次活动更新可以恢复视图。关闭 panel、切换会话或卸载会取消订阅，并使未完成的读取失效。流中断时会显示错误；重新打开 panel 会建立新订阅并读取当前状态。
