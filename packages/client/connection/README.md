...
Readiness has a bounded deadline. Expiry aborts the generation and starts backoff even if the carrier has not settled; late readiness from that generation cannot publish Host state. Stopping immediately cancels the current generation and its retry wait, then waits for all started sources to finish cleanup, including generations retired by a readiness timeout. A subsequent start owns a separate loop, so an earlier loop cannot resume alongside it. Source withdrawal and plugin disposal also await retiring owners; source implementations must settle after cancellation for disposal to complete.
