...
Readiness 有明确的等待期限。到期会中止 generation 并开始退避，即使 carrier 尚未结算；该 generation 迟到的 readiness 不能发布 Host 状态。停止会立即取消当前 generation 和重试等待，然后等待所有已启动的 source 完成清理，包括因 readiness 超时而退出的 generation。再次启动拥有独立的循环，因此先前的循环不能与它并行恢复。撤回 source 和卸载插件也会等待正在清理的 owner；source 实现必须在取消后结算，卸载才能完成。
