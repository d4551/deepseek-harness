...
在菜单内，上下方向键与 Home/End 在当前菜单的条目间移动。右方向键、Enter 或空格打开子菜单并聚焦首个条目；左方向键或 Escape 返回父条目。子菜单的无障碍名称来自该父条目。子菜单包含键盘焦点时，指针移出不会将其关闭。

`Input` 根据原生 `aria-invalid` 属性展示共享的焦点与校验样式。通过 `aria-describedby` 关联字段说明，包括密码输入框。Markdown 链接在未悬停时也显示下划线，重连横幅会向辅助技术通告状态。

### 对话流行

`PanelTable` 使用本地化列标题和无障碍表名展示相关记录。使用方提供原生表体行与行标题；该组件统一管理间距、边框与排版。
...
需要紧凑标签与摘要时，[`extractMarkdownPlainText`](src/markdown/plain-text.ts) 会在移除 Markdown 展示语法的同时保留链接标签、图片替代文本、代码文本与字面 HTML。它分隔各个块，包括脚注中的段落与代码，避免相邻词语粘连。默认的 `all` 模式返回完整投影；`first-line` 返回首个可见行；`first-paragraph` 选择首个非空段落，不存在段落时返回首个可见行。没有可见文本的内容返回空字符串。
