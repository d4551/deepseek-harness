/**
 * Durable-source to view-value projection shared by every Conversation target:
 * context provenance, presentation form, Assistant block classification, and
 * display-safe failure fields. Targets differ in what they render from these
 * values, not in how a logged source maps onto them.
 * @module @deepseek-ai/dsh-client-ui-conversation/src/client/projection/event-projection
 */

import type { ContentBlock, StreamChunk } from '@deepseek-ai/dsh-llm/types'
import type { ContextProvenanceView, KnownContextForm } from '../contract/context-provenance.ts'
import type { AssistantBlock } from '../contract/records.ts'
...
/** Forms a target presents structurally; unknown merge-extensible values remain opaque. */
...
/**
 * Project a durable message source to a transcript row's role and producer label.
 * @param source - Logged `user/message` source.
 * @returns Role and label the target renders.
 */
...
/**
 * Classify finalized Assistant content for target rendering.
 * @param content - Core content blocks.
 * @returns View blocks in source order.
 */
...
/**
 * Classify one finalized Assistant block for target rendering.
...
/**
 * Create the initial view block for one streamed Assistant block kind.
...
/** Display-safe failure fields retained by target projections. */
...
/**
 * Convert a durable failure to locale-independent fields safe to display.
...
/**
 * Whether a stream chunk carries visible model output for first-token timing.
