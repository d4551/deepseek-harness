...
describe('DeepSeek request-image pricing', () => {
  ...
  it('uses the configured pixel budget and encoded-byte target together', () => {
    const first = ref('first', 1920, 1080, 1024)
    const second = ref('second', 1920, 1080, 1024)
    const options = resolveAdapterOptions({
      models: [{ ...VISION_MODEL, imagePixelBudget: 640_000, imageMaxBytes: 16 }],
      maxRequestFilesBytes: 16,
      imageOffloadByteQuantum: 1,
    })
    expect(deepSeekImageRequestPricing(options, 'vision').priceImages([first, second])).toEqual([
      { visualTokens: 0, text: offloadedImageText(first) },
      { visualTokens: 407, text: requestImageHandleText(second, { width: 1066, height: 600 }) },
    ])
  })
