...
Image pricing must return one defined price per occurrence and a non-negative safe integer visual token count. `measure()` throws on missing or invalid prices and on a route-priced surface total outside the safe integer range; consumers receive no measurement from that call.
