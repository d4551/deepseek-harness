...
第一次请求包含一条持久基线消息：先是用户全局 `$DSH_HOME/AGENTS.md`，再按从宽泛到具体的顺序包含项目指令链——从项目根目录到会话工作目录的每个目录中所有现有候选文件。去除空白后内容一致的同级文件只渲染一次，因此复制了 `AGENTS.md` 的 `CLAUDE.md` 不会被重复加载。当成功的 `read`、`write` 或 `edit` 调用到达会话记录的任一工作区根中更深的目录后，下一次请求会包含新适用的指令文件；已改变的文件会替换其内容，消失或成为较早候选文件重复项的文件会产生移除通知。
...
带有附加工作区根的会话也会发现每个根的指令。每个附加根都是自己的项目：由它自己的标记决定其链在哪里终止，因此嵌套在更大检出内的根停在它自己的标记处，而不是外层的标记处。主根的链在前，附加根按会话记录它们的顺序跟随，这也是它们到达模型的顺序。它们的文件以绝对路径显示，因为相对主项目根写出的路径会与另一个根中的同名文件相撞。不含指令文件的根不贡献任何内容，已被主链加载的路径不会加载第二次。记录下来的根集合是基线身份的一部分，因此会话中途新增一个根会追加一份替换基线，而重新记录同一集合不追加任何内容。刷新遵循同一组根：一次 touch 会被路由到包含它的最深已记录根，因此记录在另一个根内部的根仍拥有自己的文件，而该 touch 发现的嵌套指令，其键与显示方式与该根的基线文件完全一致。相对路径被路由到主根——模型书写相对路径所依据的基准——而不在任何已记录根之下的 touch 不会发现任何内容。
