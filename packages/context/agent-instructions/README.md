...
A session with additional workspace roots discovers each root's instructions too. Every additional root is its own project: its own markers decide where its chain stops, so a root nested inside a larger checkout stops at its own marker rather than at the enclosing one. The primary root's chain comes first and the additional roots follow in the order the session recorded them, which is the order they reach the model. Their files display as absolute paths, because a path written relative to the primary project root would collide with a same-named file in another root. A root that holds no instruction file contributes nothing, and a path already loaded by the primary chain is not loaded twice. The recorded root set is part of the baseline identity, so adding a root mid-session appends a replacement baseline while re-recording the same set appends nothing. Refresh follows the same roots: a touch is routed to the deepest recorded root containing it, so a root recorded inside another owns its own files, and the nested instructions that touch discovers are keyed and displayed exactly as that root's baseline files are. A relative path is routed to the primary root, the base the model writes relative paths against, and a touch under no recorded root discovers nothing.
