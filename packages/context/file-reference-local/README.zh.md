...
当 `@file` 补全应发现 Harness 宿主自身的文件系统——即随附 `read` 工具所操作的命名空间——时，挂载此提供方。每个 agent 的工作区从该会话的工作目录，连同该会话记录的每个附加工作区根一起建立索引；两者都没有时回退到宿主进程目录。
...
在宿主 UI 中输入 `@` 会为指定 agent 返回至多 `maxResults` 个排序路径候选。包含 `/` 的查询在每个拥有该目录的根中直接列出其条目；裸查询对覆盖全部根的有界递归索引做模糊排序。目录候选以尾斜杠保持 mention 开放。任何工具结果之后，该 agent 的索引会被标记为陈旧：下一次查询仍由它作答，其替代品在后台构建，因此重建不会挡在光标前面。

来自会话主根的候选以相对该根的路径给出，即面向模型的指引所描述的形式。来自附加根的候选以绝对路径给出，因为相对根的路径会与另一个根中同名文件冲突；输入或选中这些绝对路径之一，即可继续列出该根下的目录。排序始终按候选在其自身根内的路径打分，因此根自身的位置从不决定匹配。会话记录了不同的根集合后，下一次查询会得到全新索引。
...
提供方为每个 agent 维护一个可复用的 `WorkspaceFileSearch`，覆盖 `sessionWorkspaceRoots` 从该会话日志折叠出的每个根——先是 `cwd`，然后是最后一个 `workspace/roots` 事件中的各个根。目录范围查询（`a/b/...`）在每个根中列出实时目录状态，裸模糊查询共享一次有界递归遍历，该遍历按广度优先走遍所有根，因此第一个根再深也无法在触及后续根之前耗尽整个条目预算。只有一个工作区的首次裸查询会等待该遍历；`tool/result` 事件把已完成的条目标记为陈旧，下一次裸查询在替代品构建期间继续由它作答。由于根是在读取时折叠的，客户端在会话中途改变的集合会让为上一个集合构建的索引退役。模型指引是按 agent 的提示词段，仅在指定 agent 拥有 `read` 工具时贡献；agent 释放时会同时释放索引与提示词 fiber。
...
`list(agent, query, signal)` 要么列出某个目录的条目，要么读取共享的有界索引，对候选排序（精确、前缀、子串，再到子序列得分，目录有加成，均按候选在其自身根内的相对路径打分），并按确定性顺序返回至多 `maxResults` 个。得分相同时依次按种类、路径长度和路径文本排序，最后以根的位置决胜，因此两个根持有相同路径时主根在前。`tool/result` 事件把指定 agent 的索引标记为陈旧，之后的裸查询因此观察到全新目录树。不可读或已排除的子目录不贡献候选，而不可读的根目录则让该次遍历失败：一次瞬时故障不得用不完整的索引覆盖仍然有效的条目。
...
- **所有根共用一份条目预算**：`maxEntries` 限制的是整个索引而非每个根，因此根越多越大就越早触顶。遍历按广度优先走遍各根，因此预算先花在每个根的浅层路径上，再轮到任何根的深层路径。
- **共享路径只归属一个根**：两个根都能到达的目录只建立一次索引，归属于先到达它的那个根，并按该根的形式呈现。因此嵌套在另一个根内部的根会以绝对路径给出其文件。
- **任一根不可读即让遍历失败**：只要有一个根读不到，索引就会缺少该根的条目，因此整次重建都被丢弃，由先前的条目继续作答，直到该根恢复。
