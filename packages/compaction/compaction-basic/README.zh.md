...
`dsh-compaction-basic` 让长时 agent 会话在接近模型上下文上限时仍能正常工作。token 压力上升时，它会自动把对话最旧的部分压缩为摘要、保持近期部分完整，并持续压缩到远低于触发点的水位线，使相邻步骤不会反复触发；上下文溢出错误发生后，它会压缩并重试。范围选择会跳过一个已有检查点、只摘要更新内容，并把两个或更多检查点合并回一个。你也可以通过 `dsh-command-compact` 的 `/compact` 按需压缩，并挂载 `dsh-compaction-tool-result-pruner` 先修剪超大工具输出。压缩的代价是一次额外的模型请求，它读取所选历史并写出摘要；只有摘要文本会被保留。它只压缩派生历史——无法缩减系统提示词、工具或会话前缀，也无法拆分单个不可分单元（例如一次超大工具调用）。
...
| `targetRatio` | `0.85` | 滞后水位线：继续向 `floor(thresholdTokens × ratio)` 压缩，使一次压缩落在远低于触发点的位置，而非刚低于触发点。 |
...
配置错误会快速失败：未知设置、重复的按模型覆盖、两种保留形式同时出现，或比例保留量不低于阈值，都会在加载时拒绝插件。比例保留量不低于 `thresholdRatio × targetRatio` 时也会在加载时拒绝，因为保留量不得触及滞后水位线。任何绝对 `retainTokens` 预算——顶层或按模型——不低于其阈值、或不低于其目标水位线时，都会在该模型首次使用时失败，因为这些比较需要模型的上下文大小。

### 压缩运行时会发生什么

最旧的平衡范围会被替换为一条摘要消息，近期尾部保持逐字不变；对话从摘要继续。范围选择具备检查点感知：当恰好一个已有检查点位于表层头部时，压缩会从其后开始、只摘要更新内容，留下两个检查点，直到后续合并再把它们归并；两个或更多头部检查点、或新内容太小不值得单独摘要时，则从头部合并回一个检查点。压力压缩会持续进行，直到压力降到目标水位线以下，而不只是低于阈值，因此相邻步骤不会再次触发。操作会报告压缩了多少历史项以及估算释放的 token 数。如果没有任何内容可以安全压缩——例如整个对话就是一个不可分单元——则不会有任何改变，也不会向会话日志写入任何内容。如果没有模型可以撰写摘要（既未配置目标，也还没有已路由请求），压缩会失败并给出清晰错误，提示你配置摘要提供方与模型，或先路由一次请求。
...
- **一个测量服务为每个决策定价。** 单例 `ctx.tokenMeter` 会在同一个已消费日志 revision 上测量最新规范已记录 envelope 与当前表层。路由提供方服务声明请求图片定价时，meter 会将其应用于图片历史。压力、近期尾部保留、范围选择与缩减验证使用同一套路由定价的节点数值；已记录的替换影子价仍使用与路由无关的启发式规则，使纯投影 fold 保持一致。
- **检查点感知的范围选择与滞后水位线。** 范围选择绝不拆分工具调用／结果对，在较新内容值得单独调用时跳过一个头部检查点、只摘要该内容，并把两个或更多头部检查点合并回一个。压力压缩持续进行，直到压力降到 `targetRatio × threshold` 以下，而非刚低于触发点就停止，因此一次落地不会在下一步再次触发。
...
- **摘要复用提供方的热前缀。** 逐字回放上次已路由请求的系统提示词、工具与已遮蔽区域消息，使辅助调用成为会话的真正前缀，因此只有尾随指令与摘要输出未缓存。压缩非头部范围会放弃该复用。
- **`summarize()` 是唯一的子类钩子。** 基于模板或远程摘要器的子类可以覆盖它，同时压力、保留、被引用的源事件、缩减验证与已遮蔽 token 计量仍由 token meter 负责。

### 自动触发与溢出恢复

当 `auto: true` 时，串行 `agent/pre-step` listener 会在请求派生前检查压力：它通过 `ctx.tokenMeter` 为最新持久路由请求 envelope 定价，当压力越过路由模型的阈值时，先剪枝，再朝滞后水位线压缩平衡范围。第一次压缩在较新内容值得单独调用时跳过该头部检查点，否则合并；后续压缩从头部合并。摘要未缩小其遮蔽范围时，压缩会在下一轮转为合并，而不是中止。`agent/request-error` listener 响应提供方确认的 `CONTEXT_WINDOW_EXCEEDED`：它绕过常规阈值与保留策略，尝试一次最大平衡头部缩减，并且只在表层替换 generation 前进后才授权重试。取消全程保持最终决定权。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：`BasicCompactionEngine` 与入口点分发 |
| [`src/selection.ts`](src/selection.ts) | 检查点感知的范围选择、定价快照、稳定性检查 |
| [`src/transaction.ts`](src/transaction.ts) | 先记录标记的压缩事务 |
| [`src/lock.ts`](src/lock.ts) | 持久压缩锁的入口状态与断言 |
| [`src/summarizer.ts`](src/summarizer.ts) | 默认 `ctx.llm.stream()` 摘要、检查点框定、缩减验证 |
| [`src/config.ts`](src/config.ts) | 加载时验证与路由模型策略解析 |
| [`src/auto.ts`](src/auto.ts) | 自动压力与溢出恢复 listener |
| [`src/manual.ts`](src/manual.ts) | 空闲接纳的手动压缩入口点 |
| [`src/target.ts`](src/target.ts) | 策略与摘要共享的路由目标解析 |
| [`src/schema.ts`](src/schema.ts) | 面向 Loader 的配置 schema |
...
- **摘要失败会保留最新持久表层**——任何替换前，自动路径会记录警告，并携带完整超预算历史继续。未缩小其遮蔽范围的摘要会在配置的重试预算内作为合并压缩重试，之后才会触发该警告。如果剪枝已落地，后续摘要失败会从该持久剪枝表层继续。因达到 `maxTokens` 而发生的摘要截断（隐藏推理 token 可能会耗尽该额度）遵循同一规则。
...
- **默认比例，尚未决定**——`thresholdRatio: 0.8`、`targetRatio: 0.85` 与 `retainRatio: 0.16` 是固定默认值；存在通过 `modelPolicies` 进行的按模型调优，但没有基于语料的理想值指引记录。
