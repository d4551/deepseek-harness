...
import * as ApprovalAssessor from '../src/index.js';
/** Minimal agent stand-in with a real session for event lookups. */
function sessionAgent(id, seedEvents = []) {
...
describe('user instruction quoting', () => {
    it('includes the last user instruction in the rejection context', async () => {
        ...
        const injected = session.events.filter(e => e.type === 'user/message'
            && e.data.source?.kind === 'plugin');
        expect(injected.length).toBeGreaterThanOrEqual(1);
        const lastInjected = injected.at(-1);
...
//# sourceMappingURL=approval-assessor.spec.js.map
