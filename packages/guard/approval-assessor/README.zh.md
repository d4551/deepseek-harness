...
每个 `dsh-base` profile 都在产品审批应答者之前挂载本插件。它没有部署设置、存储设置、覆盖值或退出开关。每个请求都必须携带通过审计的非空理由，之后下游应答者才能作出决定。
...
插件在面向用户的应答者之前监听 `approval/request` 瀑布；`dsh-base` 在添加那些应答者的层之前挂载它。理由缺失或匹配内置规避工作模式时，插件注入来源为 `plugin: approval-assessor` 的重定向，不调用 `next()` 而直接判定 `rejected`。非规避理由在审计后委托。重定向经由智能体收件箱作为稍后的 `user/message` 进入日志。`./invariant` 伴生件确保会话中已提交的重定向条数永不超过被拒审批决定的条数。

审计适用于每个审批请求。缺少理由和内置规避工作模式都会拒绝。没有人类消息的会话仍会收到拒绝，只是不附带指令引用。
