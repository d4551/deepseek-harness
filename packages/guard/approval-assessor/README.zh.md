---
description: "默认审批筛查：拒绝缺少理由或命中规避工作模式的理由，并将模型重定向到用户指令。"
...
在应答者作出决定前筛查工具审批请求。筛查默认启用，会拒绝缺少理由，或命中跳过、推迟、弱化用户已授权工作规则的理由。拒绝会将模型重定向到最近的人类指令。通过筛查的非空理由会继续交给已配置的应答者。
...
| 字段 | 默认值 | 含义 |
|---|---|---|
| `enabled` | `true` | 拒绝缺失及命中规避工作模式的理由；`false` 会原样委托仍有效的请求。 |
...
理由缺失或命中规避工作短语的请求会被拒绝，模型收到一条插件署名、引用用户最近指令的消息。只有人类消息才算作该指令。`bash`、`pwsh`、`write`、`edit` 与其他工具使用相同的筛查策略。

筛查前已撤回的请求以 `cancelled` 完成，不产生重定向或下游决定；即使筛查关闭也如此。
...
启用的审计适用于每个审批请求。缺少理由以及内置或配置的规避工作模式都会拒绝。匹配时，对理由和配置短语都执行 Unicode NFKC 规范化、移除默认可忽略码位，并统一空白和大小写。没有人类消息的会话仍会收到拒绝，只是不附带指令引用。
...
- **仅使用规则匹配** — 同时未命中内置规则与 `extraPhrases` 条目的改述式规避会通过筛查。启用[对抗式评审者](../approval-adversary/README.zh.md)，即可在本阶段之后由模型评审授权。
