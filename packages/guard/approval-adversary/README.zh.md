...
每个 `dsh-base` profile 都在[审批筛查](../approval-assessor/README.zh.md)之后挂载评审者，默认关闭。通过 Web UI 启用：

1. 打开**设置 → 插件 → 审批流程**，将**评审者**设为**由评审者决定**。
2. 将**评审提供方**和**评审模型**都留空，以使用智能体最近记录的路由；或同时填写两个标识符，指定独立评审者。
3. 设置评审上限和附加限制，然后选择**保存**。表单会一起保存所有评审字段。

所选提供方接收完整人类指令历史、工具参数和理由。启用期间，评审者负责决定：只有明确允许才能继续，未决请求不会交给其他应答者。
