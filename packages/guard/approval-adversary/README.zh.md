...
被允许的请求如同人工一次性授权那样继续执行，模型读到一行写明评审者理由的文字。被拒绝的请求闭合失败，模型读到理由、一条"不要改写理由重新提交"的指示，以及用户最近的人类指令。超时、失败或未给出裁决行的评审遵循 `fallback`：由下一个应答者决定，或以"评审无法决定"的通知拒绝请求。评审期间被撤回的问题以 `cancelled` 完成，不附带通知。
...
插件在评估器的确定性筛查之后监听 `approval/request` 瀑布。禁用时立即委托。启用时它解析路由，把证据封装为一条 JSON 记录，追加仅记录日志的 `approval/adversary-request` 事件（携带精确的路由、系统提示、消息与 token 上限），然后在与请求信号融合的 `timeoutMs` 截止时间内通过 `ctx.llm` 流式发起评审调用。回复中的 `VERDICT:` 行决定结果；`REASON:` 行成为通知的详情。通知经由智能体收件箱作为稍后的 `user/message` 进入日志，来源为 `plugin: approval-adversary`，`notice` 形态的摘要写明裁决。`./invariant` 伴生件确保会话中的允许通知条数永不超过已授予的审批决定条数，拒绝与无法决定通知条数永不超过已拒绝的决定条数。
