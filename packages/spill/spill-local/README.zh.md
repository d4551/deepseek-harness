...
后端只负责存储细节，建立在一个原则之上：**spill 工具结果必须私有且不可重定向**。根目录私有（0700）、会话目录是稳定哈希、文件名不可预测、写入采用排他模式并在 POSIX 上仅所有者可访问。存储机制放在与 Cordis 无关的模块中，以便无需上下文即可单元测试。
...
- **Windows 上的创建不是仅属主**——`0700` 根目录与 `0600` 文件 mode 在 Windows 上并不存在，因此两者都会继承其所在目录的访问控制列表。只有清理扫描会审计访问控制列表，而且它审计的是自己扫描的根目录，而不是本后端写出的文件；当环境临时目录不是只有你的账户能到达的目录时，请把 `root` 设为这样的目录。
