...
import { requireEnvironmentPolicy, SENSITIVE_ENV_PATTERN } from '@deepseek-ai/dsh-subprocess'
import type { SubprocessEnvironmentPolicy } from '@deepseek-ai/dsh-subprocess'
 ...
 * @param policy - whether the scrubbed remote environment is retained or removed entirely.
...
export function serializeRemoteEnvironment(
  ...
  policy?: SubprocessEnvironmentPolicy,
): string {
  requireEnvironmentPolicy(policy)
  const environment = policy === 'isolated' ? new Map<string, string>() : scrubRemoteEnvironment(raw)
