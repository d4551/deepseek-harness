/**
 * Sandbox-result classification for the shell executors that confine every
 * command through `ctx.sandbox` (`@deepseek-ai/dsh-bash-sandbox`,
 * `@deepseek-ai/dsh-pwsh-sandbox`). Those providers derive the same
 * `ShellSandboxInfo` facts — `denied` and `runnerFailed` — from a settled
 * process, so the derivation lives with the seam that declares those facts
 * rather than once per provider.
 *
 * The rules are dialect-parameterized, never platform-branched: a caller passes
 * the denial signatures and {@link RunnerFailureRule}s its selected runner
 * publishes (bwrap, landlock-run, seatbelt, windows-acl-run), and this module
 * only decides what counts as evidence.
 *
 * @module @deepseek-ai/dsh-shell/sandbox-classify
 */
