...
async function setup(
  ...
  const fiber = await ctx.plugin(ToolShellPersistent, { dialect: 'pwsh', ...config })
  return { ctx, stub, fiber, owner: agent(ctx, '/workspace') }
}

describe('tool-shell-persistent: the pwsh dialect', () => {
  ...
  it('cancels and awaits a pending shell spawn when the plugin is disposed', async () => {
    ...
    const fiber = await ctx.plugin(ToolShellPersistent, { dialect: 'pwsh', backendType: 'slow' })
  ...
  it('rejects invalid config and invalid calls', async () => {
    ...
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'pwsh', backendType: '' })
    }).toThrow('backendType must be non-empty')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'pwsh', timeoutMs: 0 })
    }).toThrow('timeoutMs must be a positive safe integer')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'pwsh', maxOutputChars: 0 })
    }).toThrow('maxOutputChars must be a positive safe integer')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'pwsh', description: ' ' })
