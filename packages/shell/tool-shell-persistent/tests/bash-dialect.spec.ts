...
// Every case in this file drives the same dialect; `setup` states it once.
async function setup(
  config: Omit<ToolShellPersistent.Config, 'dialect'> = { backendType: 'stub' },
  ...
  const fiber = await ctx.plugin(ToolShellPersistent, { dialect: 'bash', ...config })
  return { ctx, stub, fiber, owner: agent(ctx, '/workspace') }
}

describe('tool-shell-persistent: the bash dialect', () => {
  ...
  it('cancels and awaits a pending shell spawn when the plugin is disposed', async () => {
    ...
    const fiber = await ctx.plugin(ToolShellPersistent, { dialect: 'bash', backendType: 'slow' })
  ...
  it('rejects invalid config and invalid calls', async () => {
    ...
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'bash', backendType: '' })
    }).toThrow('backendType must be non-empty')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'bash', timeoutMs: 0 })
    }).toThrow('timeoutMs must be a positive safe integer')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'bash', maxOutputChars: 0 })
    }).toThrow('maxOutputChars must be a positive safe integer')
    expect(() => {
      ToolShellPersistent.apply(new Context(), { dialect: 'bash', description: ' ' })
