---
description: "在 Host Team 层之后，为源码 checkout 的 Web profile 添加 Agent Teams 面板。"
kind: "package-bundle"
---
...
`@deepseek-ai/dsh-agent-team-web-profile` 是 [Agent Teams](../../subagent/agent-team/README.zh.md) 的私有 Web 层。把它放在 `@deepseek-ai/dsh-web-app` 与 [`@deepseek-ai/dsh-agent-team-profile`](../agent-team-profile/README.zh.md) 之后，即可在浏览器中显示 Team roster、任务板与 teammate 导航。移除任一 Team 层都会让稳定的 base 与 Web composition 保持不变。正式发布会排除本包，因此只能从源码 checkout 使用。
...
第一条命令提供 Team domain、生成的 Remote 方法与模型工具。第二条命令激活本包声明的 patch 及其浏览器 presentation。执行 `dsh plugin --profile web remove @deepseek-ai/dsh-agent-team-web-profile` 移除本包时，Web 层也会从 profile 的有序 bundle 列表中移除。
...
- **有序组合**——`dsh-base`、`dsh-web-app`、`dsh-agent-team-profile` 与本包必须保持这个顺序。
