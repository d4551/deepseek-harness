...
每个根目录包含以 preset id 命名的目录。要注册 `meowbao`，应提供 `<root>/meowbao/agent.cordis.yml`；若根目录指向安装目录，注册的将是该目录的直接子目录。符号链接形式的 preset 目录保留链接名称作为 id。相对插件名称和被包含的组装路径从注册目录解析，因此部署必须在这些路径提供依赖。既有会话在恢复或 fork 时，仍需要注册其记录的 preset id。
...
可等待的 `agent-preset/recompose` 生命周期允许插件在父链接移动前移除 Agent 拥有的贡献，并在移动后重建。监听器必须恰好等待一次 `next()` 才能允许转换。监听器在调用 `next()` 前拒绝会阻止重新链接；贡献会依据保留的组合恢复。
...
挂载要求宿主组装提供 `ctx.baseUrl`。包名通过 Node 原生模块加载器从该宿主解析；加载器不可用时，包导入会被明确拒绝。相对路径从 preset 目录解析，绝对路径、文件 URL 与 `cordis:` 内建插件则保留各自的解析方式。失败的挂载会等待其拥有的卸载操作；若卸载也失败，报告的错误会保留两项失败。

释放 preset 子树时，卸载操作会释放其注册记录，即使进程此后不再挂载或查询其他 preset。已释放子树的异步清理仍在运行时，注册表读取也不会返回该子树。
...
- **代际以组装文件为键。** 挂载记录组装文件的 stamp（mtime 与大小）；发现 stamp 过期的会话会开启下一个代际，而已加入的会话保持各自运行的那个代际——运行中的会话在文件被修改或删除后继续存活——被替代的代际会在其最后一个已加入会话结束时释放，若从未有会话加入则立即释放。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 服务入口：`Config` schema、settings 命名空间、名单 API、agent 加入 |
| [`src/standing.ts`](src/standing.ts) | 常驻挂载：以文件 stamp 为键的代际、加入记录、被替代代际的回收 |
| [`src/remote-failures.ts`](src/remote-failures.ts) | 名单的稳定 Remote 失败词汇 |
...
- **被替代的代际与其最后一个会话同寿**——已加入的会话保持其运行所在的代际，因此每一轮「编辑后建会话」会同时持有两个代际，各带一套 `dsh-skill-filesystem` watcher，直到旧代际上的每个会话都被释放；到那时——或从未有会话加入时立即——被替代的子树才会释放。
...
`standingKeyFor()` 把常驻 key 交给宿主读取者时不做加入，因此没有会话运行的被替代代际可能在这样的读取进行中就被释放；读取者随后在已释放的注册上什么也解析不到，只能再问一次。让读取在自身持续期间加入即可关闭这个窗口；目前尚无设计。
